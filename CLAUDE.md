...
7. **Node-level LLM routing, not agent-level.** Capability profiling (see `ops/llm_routing.md`) drives selection per LangGraph node, not per agent. Local Qwen3-8B is reserved for nodes that are pure threshold judgement, JSON extraction, token-level comparison, or short-string rewrite (Monitor anomaly detection, Verifier numeric comparison, Critic numeric trace-back, Specialist query rewrite). Cloud LLMs (DeepSeek-V3 / V4-Flash) are mandatory for any node that requires multi-fact induction, faithfulness to retrieved context, completeness judgement, or causal reasoning (Planner DAG generation, Specialist decompose / grade / generate, Reflector semantic + procedural consolidation, Conversational). Reflection is **not** a routine task and must never run on local. Cost is a first-class design metric, but capability floors override cost — see `ops/llm_routing.md` for the per-node ablate conditions and fallback paths.
