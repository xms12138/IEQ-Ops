...
| 5 对话+展台 🔧 进行中 | 问答管家(两步式按需取数、多轮+流式、真语音级联 Qwen3-TTS/ASR)+ 运维面板/Tier3 审批 + scheduler/kiosk systemd + Pi 单机 CPU 检索闭环 + 闭环 replan + 断电韧性 + 自启 kiosk + **Arduino MKR1010 真传感器接入 broker(P-034,真数据已达 Pi;ingest/read_sensors 切换待续)**;**展台本地完全体近收尾**;**远程问答待接**(cloudflared,见 Phase 5 末块)|
...
> **🆕 2026-06-14(续·Phase 3 闭环补完 — 接 replan,纯软件不等硬件):** 兑现 CLAUDE.md 原则 #2「失败的干预触发 replan 而非沉默」的另一半——`critic` 否决(Tier1/2)/ `verifier` missed 原本都是终态(END/FAILED),`replan_count` 恒 0;现接回 planner 重试,`MAX_REPLANS=2`(共 3 次规划),耗尽才 FAILED。**难点不在路由在「重试要清空上一轮」**:`subtask_results`(merge)/`failed_subtasks`(append)是并行 fan-out 的累加通道,`return {}` 会被 reducer 合并成 no-op、旧结果残留致新 plan 子任务被当「已解决」永不重派;解法 = `_ResetResults/_ResetList` 子类哨兵让 reducer **替换而非累加**(本身即 dict/list,Pydantic 通道校验仍过)——经真实 `StateGraph` 编译运行单测证实清空无报错。critic/verifier 用同一 `replans_left` 决策、`replan` 节点唯一递增计数(与路由一致);留预算时 critic 不写 ticket FAILED、verifier 不落 episodic(中途 miss 非结局)。**长跑同步**:`scheduler.resume_tick` 复查 `snap.next` 二次挂起则续注册(幂等 upsert)、`demo.py` 改 while 循环每尝试推进一次模拟。**验证 · 新 `co2_overcrowded` 场景**(20人/50m³、通风拉满仍稳态 1220ppm、执行器够不着):一次跑两条 replan 路径全覆盖——critic 否决→replan、verifier missed→replan、`replan_count=2` 耗尽→**status=failed verdict=missed**,每尝试 specialist 都重跑(证明通道确清空)、无死循环。`mypy --strict core/` 过。**诚实边界**:这是「重试到耗尽如实判 FAILED」的安全叙事,非「重试后成功关单」(高通风一窗即近稳态,翻盘需极高初值且受 LLM 目标值扰动、不易确定性复现);成功关单仍由 `co2_spike` 承载。详见 DEVLOG **P-024**。
...
> **🆕 2026-06-26(Phase 5·展台真机收尾两连):** ① **语音级联 Pi 真机首通(P-033)**——Lite OS 补 PipeWire 音频栈 + emoji 字体,chromium 重启后接上,push-to-talk→DashScope STT→DeepSeek→TTS→扬声器全链路通,固化 `ops/deployment/setup-audio.sh`。② **Arduino MKR1010 真传感器接入打通到 broker(P-034)**——固件 `sensing/hardware/mkr1010_node/` 真机烧录跑通(板挂 Windows 由作者在 Arduino IDE 烧;踩平 SAMD 无 `dtostrf`→改 `String`、补 `ArduinoMqttClient`+`SparkFun SCD30` 库),**实际硬件 = SCD-30 + Grove Light/Sound v1.6**(非旧方案 SCD40/BH1750/DHT22;LM393 数字声音版否掉)经 WiFi/MQTT 发 `ieq/readings`;Pi 装 Mosquitto 2.0.21 + 配 `listener 0.0.0.0`,**真数据已达 broker**(`{co2:682,temperature:35.8,humidity:31.2,light_raw:669,sound_raw:249}`),**客户端隔离这次没挡**(MKR↔Pi 同网段直连)。**下次接力(硬件线收尾,纯 Pi 软件、作者不碰板子):** `sensing/ingest/` 订阅 writer(标定 light_raw/sound_raw→lux/noise_db + SCD30 温度减自发热偏移)→ `read_sensors` 加 `SENSOR_SOURCE=hardware` 开关 → 真读数进 `sensor_readings`、上层零改当场用上;标定极值 + 温度偏移待采。详见 DEVLOG P-033/P-034。

> **🆕 2026-06-26(续·真实数据全面接入 + 长期跑 P-035):** 把展台从 sim 切到 **Arduino 真实数据 7×24**(承接 P-034)。**Hybrid 数据模型**(作者选择器拍板):真实数据常态 + 注入临时覆盖。**真实态**(`sensor_source=hardware` 无 override)monitor 检测真异常→诊断→新 `advisory_close` 节点(置新状态 `OBSERVING`,**不跑假执行器闭环、不 FAILED、不堆 Tier3**),环境恢复 monitor `_reconcile_recovered` 自动关单;**注入态**置 `sensing/override` 跨进程标记→read_sensors 回落模拟器走完整闭环→终态 `clear_if_resolved` 回归真实(TTL 兜底)。新增 `sensing/ingest`(paho writer)+ `sensing/calibration`(温度减自发热偏移 9.4、光声 raw→lux/dBA 两点线性、**未标定→in-band safe 不乱报**)+ read_sensors 三态。部署:`ieq-ingest.service`(venv python,非 uv run,避免 sync 卸 paho)、Pi `.env` SENSOR_SOURCE=hardware。**实测**:read_sensors 走真实值、scan 不误报、注入 override 切换(1300↔781)、ruff+mypy 绿。**长期跑已起**(ingest/scheduler/web systemd enable+Restart,开机自启)。**待续**:光声四极值标定(暗/亮静/响)+ 温度精调。详见 DEVLOG P-035。
...
**🔜 Phase 5 对话线(2026-06-08 起当前重心):** 问答管家 MVP ✅ → 多轮+流式 ✅(commit `1ebadd8`)→ 真语音级联 ✅(P-021)→ butler 输出修整 + 长期运行加固 ✅(2026-06-09)。**下一缺口:** ① RAG 标准条款问答(要标准原文/出处时)② 运维面板 ✅只读流+异常注入(P-025)+ ✅ Tier3 审批(P-026,作者拍板**登记式**非 graph-resume:无 actuator 域 approved→人工已处置关单)③ 长期运行剩余加固(#3 审批超时 / #4 monitor 纯确定性 / #7 语音清理 / #8 web auth)+ ✅ scheduler/reflection 的 systemd 模板(P-025)。**硬件:** 按定稿方案落码(Arduino MKR 1010 固件 + `sensing/ingest` writer + `read_sensors` 的 sim/hardware 切换,两空目录待写)。
...
> 1. ✅ **[Phase 3 缺口·健壮性核心] 接 replan**(2026-06-14 完成,P-024)—— critic 否决(Tier1/2)/ verifier missed 接回 planner 重试,`MAX_REPLANS=2` 耗尽才 FAILED;reset 哨兵清空 `subtask_results`/`failed_subtasks`;`co2_overcrowded` 场景一跑两路径全覆盖。与 P-020 的 Tier3 escalate(无自主解→交人)互补。
...
> 5. ✅ **[Phase 5·硬件] 真实数据全面接入(P-034 固件→broker、P-035 ingest→read_sensors→长跑)** —— Arduino 真数据 7×24 入库,Hybrid(真实常态 + 注入覆盖演示),长期跑已起。**剩纯标定**:光声四极值(暗/亮静/响)+ 温度偏移精调,填 `.env` 即上线,不阻塞长跑。
...
- [~] **真实硬件接入(2026-06-26 真机接入中;实际硬件偏离 2026-06-08 方案,见下条 📍 + DEVLOG P-034)**:
  - [x] **Arduino 固件**(MKR WiFi 1010,`sensing/hardware/mkr1010_node/`)——**实际传感器 = SCD-30(CO2+温+湿,I2C,一颗覆盖 air+thermal)+ Grove Light v1.1(A0,模拟)+ Grove Sound v1.6(A1,模拟峰值)**,非方案里的 SCD40/BH1750/DHT22(声音一度想用 LM393 数字版,否掉:数字只给阈值二值、给不出连续 `noise_db`,会废掉 WELL 分级 + 时序统计)。WiFiNINA + ArduinoMqttClient 每 5s 发 JSON 到 topic `ieq/readings`,key 对齐 `SENSOR_COLUMNS`(co2/temperature/humidity)+ **`light_raw`/`sound_raw` 原始 0–1023**(标定刻意留 Pi、免反复烧板)。**2026-06-26 真机烧录跑通**(踩平 SAMD 无 `dtostrf`→改 `String`):SCD30 ready + WiFi 连上 + 数据真达 broker
  - [x] **RPi 网关 Mosquitto**:`apt install mosquitto`(2.0.21)+ `/etc/mosquitto/conf.d/ieq.conf`(`listener 1883 0.0.0.0` + `allow_anonymous true`,LAN 无鉴权、web auth 同列加固 #8);`enabled` 自启。实测 **MKR(.153)↔Pi(.152)同网段直连成功——客户端隔离这次没挡 MQTT**(免上手机热点)
  - [ ] **`sensing/ingest/` writer(下次接力第一件)**:订 `ieq/readings`(paho)→ 标定 `light_raw`/`sound_raw`→`lux`/`noise_db` + 减 SCD30 温度自发热偏移 → `sensing.history.record_reading()` 写 `sensor_readings`(**弃 InfluxDB**,理由见下)
  - [ ] **`read_sensors` 切换**:`mcp-sensor-server.read_sensors` 加 `SENSOR_SOURCE=sim|hardware` env 开关——hardware 读 `sensor_readings` 最新行、sim 仍读模拟器(**两者并存**:真实感知 + 注入场景演闭环);**工具名+输出形状不变 → 上层 multi-agent 零改动**
  - [ ] **标定值待采**:light_raw 暗/亮极值、sound_raw 静/响极值(映射伪 lux/伪 dB 对齐 WELL 320 lux / 50 dBA);SCD30 温度偏移(实测 35.8°C 明显偏高=自发热,`setTemperatureOffset` 或 Pi 侧减偏移)
  - [~] **麦克风留 RPi**:butler 语音用 USB mic(P-033 真机通);**acoustic 声学感知改用 Grove Sound v1.6 模拟值**(取代原"RPi 算 dB"方案,作者接受相对值)
