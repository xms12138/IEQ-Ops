...
**🔜 Phase 5 对话线(2026-06-08 起当前重心):** 问答管家 MVP ✅ → 多轮+流式 ✅(commit `1ebadd8`)→ 真语音级联 ✅(P-021)→ butler 输出修整 + 长期运行加固 ✅(2026-06-09)。**下一缺口:** ① RAG 标准条款问答(要标准原文/出处时)② 运维面板 ✅只读流+异常注入(2026-06-14,P-025);**剩 Tier3 审批**(需 graph-resume)③ 长期运行剩余加固(#3 审批超时 / #4 monitor 纯确定性 / #7 语音清理 / #8 web auth)+ ✅ scheduler/reflection 的 systemd 模板(P-025)。**硬件:** 按定稿方案落码(Arduino MKR 1010 固件 + `sensing/ingest` writer + `read_sensors` 的 sim/hardware 切换,两空目录待写)。
