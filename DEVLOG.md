# DEVLOG — 开发问题与踩坑复盘

开发过程中遇到的**问题**的专项记录:现象 → 根因 → 解决(或状态)→ 教训。供作者日后复盘项目、答辩举证、求职讲故事用。

**与其他文档的分工:**
- `EXECUTION_PLAN.md` 记**进度**(做了什么、到哪个 Phase)。
- 本文件记**问题**(踩了什么坑、为什么、怎么爬出来的、学到什么)。
- 二者互补:进度是「正面叙事」,本文件是「失败与修正的证据链」——后者往往才是复盘和面试时最有价值的部分。

---

## 记录约定

- 一条问题一个条目,编号 `P-001` 递增,**正序追加在末尾**(最新在底部)。
- 状态图例:✅ 已解决 · 🔧 进行中 · ⏳ 待处理。
- 未解决的(🔧/⏳)同时登记到下面的「未决速查」,复盘时一眼可见。
- 模板:

  ```
  ### P-0XX · <Phase> · <一句话标题>  <状态 emoji>
  - **现象**:观察到的异常表现。
  - **根因**:为什么会这样。
  - **解决** / **状态**:怎么修的;未修则写计划。
  - **教训**:下次怎么避免 / 可复用的原则。
  - **关联**:file:line · commit · memory · bench 任务。
  ```

---

## 未决速查

| 编号 | 状态 | 标题 |
|---|---|---|
| P-009 | ⏳ 待处理 | verifier 取 subtask_result 与 action 选取逻辑不一致 |
| P-010 | ⏳ 待处理 | e2e 对照 +10pp 全靠 acoustic 单域(简单任务封顶,≥10pp 未稳) |
| P-014 | 🔧 部分延后 | airquality bench 种子按 WELL 重写(retrieval 0.60 + generate/e2e 硬编码 1000/ASHRAE),并入扩 200 |
| P-016 | 🔧 待对齐 | thermal/lighting/acoustic 真 corpus 已接入(WELL v2 三 concept,824 chunks);thresholds 占位 + 三域 bench 种子 + contextual 待下次对齐 |

> P-008 已于 2026-05-31 解决(generate/v4),P-014 阈值对齐已 ✅、bench 种子重写并入扩 200,均见下。

---

## 问题记录

### P-001 · Phase 0 · 6GB 显存三方共驻不可行 + retrieve 跑 CPU 爆预算  ✅
- **现象**:计划让本地 Qwen3-8B + RAG retrieve + reranker 三方共驻一张 6GB 卡;沿用 `douluo` 旧项目经验把 retrieve 放 CPU。
- **根因**:`vram_spike.py` 实测——Qwen 单独就吃 5.6GB(已 CPU offload 2.1GB),三方共驻不可行;CPU retrieve 实测 3.76s,爆 <500ms 预算 **7.5x**。
- **解决**:dev 期 GPU 只跑 retrieve(4.0GB,fp16 98ms);Qwen 单独占卡;Phase 6 用 ollama `keep_alive` 分时复用。推翻 douluo 的 CPU 策略,定稿写入 `TECH_STACK.md`。
- **教训**:跨项目搬经验必须在**目标硬件**上实测;延迟/显存预算要用数字卡死,不能凭直觉。
- **关联**:`ops/scripts/vram_spike.py` · `TECH_STACK.md` · memory `project_vram_strategy`

### P-002 · Phase 2 · BGE-M3 safetensors 离线缓存坑  ✅
- **现象**:offline 模式加载 `bge-m3` 直接报错。
- **根因**:safetensors 权重需先在线下载预热到 HF 缓存,之后才能 offline 加载。
- **解决**:首次在线 warm cache,之后可离线;加载方式对齐 `vram_spike`(SentenceTransformer + safetensors / 手写 transformers reranker),而非 douluo 的 CPU/CrossEncoder。
- **教训**:HF 模型要 offline 部署,先在线预热缓存是前置步骤,别在断网环境第一次拉权重。
- **关联**:`rag/retrieve.py` · memory `project_phase2_retrieval`

### P-003 · Phase 2 · v4 reasoning 模型偶发返回空 content  ✅
- **现象**:Specialist 子图 `decompose/grade/generate` 偶发无端失败,响应体为空。
- **根因**:v4 reasoning 模型把 token 预算全花在 `reasoning_content`,偶发返回空的 `message.content`。
- **解决**:子图四节点加 `extra_body={"thinking":{"type":"disabled"}}`(它们是 JSON/短串结构化任务,非推理深度任务);温度固定(decompose/grade=0、generate=0.2、rewrite=0.3)。延迟 ~11s→~3s,判断可复现。Planner **保留** thinking(DAG 分解确实需要推理)。
- **教训**:reasoning 模型用于「结构化输出」节点要显式关 thinking;否则空响应 + 不可复现 + 高延迟三连。是否开 thinking 应按**节点任务性质**决定,不是一刀切。
- **关联**:`agents/specialists/builder.py:55` `_NO_THINK`

### P-004 · Phase 2 · Send 并行 fan-out 写父 state 冲突  ✅
- **现象**:波次并行 fan-out 多个 specialist 同时写父 state,LangGraph 抛并发更新错误。
- **根因**:多分支并发写同一个**非 reducer** channel,LangGraph 直接拒绝。
- **解决**:`subtask_results` 配 merge reducer(`_merge_results`)、`failed_subtasks` 配 add reducer(`operator.add`);`status` 不在并行分支里写,留到 critic 之后统一写。
- **教训**:LangGraph 里凡是会被并行分支写的 channel,必须配 reducer;并发写裸 channel = 报错。
- **关联**:`core/state.py:44` `_merge_results` · `agents/specialists/builder.py` `run_specialist`

### P-005 · Phase 2 · generate 跨节点契约(1):正文阈值 ≠ target_value  ✅
- **现象**:CriticAgent 上线后,co2 关单率从 ~3/3 掉到 ~1/3。
- **根因**:generate 正文说"通风到 below 800ppm",但 `expected_outcome.target_value` 填 900;Verifier 拿 `target_value` 核验 → 与正文不一致,critic 正当否决。
- **解决**:generate prompt bump **v2**,规则 4 强制「正文阈值 = `target_value`」(挑一个数,两处一致)。改后 critic approve → 关单 3/3。
- **教训**:LLM 同时产「自然语言」和「结构化数值」时,两者会各自漂移;prompt 必须把它们**锚定到同一个数**。
- **关联**:`ops/prompts/specialist/generate/v2.md` · commit `af4abb9`

### P-006 · Phase 2 · generate 跨节点契约(2):target_metric 用了自然语言名  ✅
- **现象**:demo 四 domain 实跑,lighting/acoustic 的 generate 把 `target_metric` 写成 `illuminance`/`noise level` → critic 正当否决 + verifier `KeyError` 隐患。
- **根因**:critic 查物理范围表、verifier 查 `read_sensors` 都按 metric **键名**索引;自然语言名不是合法键。
- **解决**:bump **v3**,规则 5 强制 `target_metric ∈ {co2,temperature,humidity,lux,noise_db}`。
- **教训**:任何「跨节点当 dict 键用」的字段都要锁定枚举,绝不能让 LLM 自由命名。
- **关联**:`ops/prompts/specialist/generate/v3.md` · `core/state.py` `SENSOR_DOMAIN` · commit `30f5a75`

### P-007 · Phase 2 · ReWOO 占位符 hydrate 因花括号漂移断裂  ✅
- **现象**:依赖型子任务(S2 `depends_on` S1)拿不到上游诊断,空跑 rewrite×3;ReWOO 依赖传递实际断裂。
- **根因**:planner LLM 偶发照抄元符号 `#{subtask_id}`,产出带花括号的 `#{S1}.diagnosis`;原正则 `#(\w+)\.` 不认这个变体。
- **解决**:放宽正则为 `#\{?(\w+)\}?\.`,容忍花括号漂移;重测 S2 占位符正确填入 S1 真诊断。
- **教训**:依赖 LLM 严格遵守符号格式不可靠;**解析端**要主动容忍常见漂移(防御式正则),比靠 prompt 纪律便宜也安全。
- **关联**:`core/graph.py:91` `_REWOO_REF` · commit `30f5a75`

### P-008 · Phase 4 · generate co2 自洽 flaky  ✅
- **现象**:co2 generate 多次采样,critic 稳定否决约 30%(2026-05-31 `--n20` baseline:自洽率 0.70、否决率 30%;05-30 手测一度 62–67%——**高方差**指标);thermal 对照 0.95–1.0。
- **根因**:非零温度(0.2)下,generate 偶发在正文多嘴**预测「干预后会降到多少」**——复述 corpus `ops-note` 的"降 150–300ppm",从 1300 起降只到 1000–1150,与自己声明的 `target_value=800` 矛盾 → critic 正当否决。**纯自洽矛盾**,`mean_hit=1.0`(groundedness 无问题)。v3 规则 4 只管「正文阈值 = target_value」,没堵住「额外的降幅预测」这个口子。
- **解决**:bump `generate/v4` 加**规则 6**——禁止正文预测降幅/到达值(`will drop by` / `will settle around`),只承诺单一 `target_value`,降多少交给 Verifier;同时明确**保留**引用标准阈值的权利(护 groundedness)。builder 指向 v4。**delta(`--n20` 同环境):co2 自洽率 0.70→1.00(否决率 30%→0%)· mean_hit 1.0→1.0(未伤 groundedness)· thermal 0.95→1.0(未误伤对照)**;+0.30 远超 `--n20` 噪声 ±0.07。
- **教训**:即使 prompt 已要求「自洽」(v3 规则 4),非零温度下 LLM 仍会「多嘴」引入矛盾数字;约束要**具体到禁止某类输出**(禁止预测降幅),而非泛泛说「保持一致」。另:修 prompt 必须 before/after **同环境同 `--n`** 量化——高方差指标尤甚,4 天前的数字不可直接当对照。
- **关联**:bench `L2-generate-co2-flaky` · `ops/prompts/specialist/generate/v4.md` 规则 6 · `agents/specialists/builder.py` 指向 v4

### P-009 · Phase 2 · verifier 取 subtask_result 与 action 选取逻辑不一致  ✅
- **现象**:(潜在,尚未触发)`verifier.py:44` 用 `next(iter(state.subtask_results.values()))` 取**第一个**子任务结果,而 `action`/`autonomy_gate` 用 `state.primary_result()`(domain 匹配异常 sensor 的那个)。
- **根因**:单子任务 co2 场景两者恰好相同,所以一直没暴露;但多子任务 DAG 且 dict 里第一个不是 primary 时,verifier 会去核验一个 advisory 子任务的指标。
- **解决**:✅(2026-05-31)verifier 改用 `state.primary_result()`,与 `action`/`autonomy_gate` 同源;None 时返回 FAILED(与那两个节点一致),不再 `next(iter())`。
- **教训**:同一个「主结果」的选取逻辑应集中一处(`primary_result()`),各节点共用,避免各取各的埋下不一致。
- **关联**:`agents/verifier.py` `run()` · `core/state.py` `primary_result()` · commit `ffeeca0`

### P-010 · Phase 4 · e2e 对照表机制跑通,但 +10pp 全靠 acoustic 单域撑(简单任务封顶)  ⏳
- **现象**:`--compare` 四域 e2e(同一 anomaly,系统 `planner`+AgenticRAG vs 朴素 ReAct,同一 `CriticAgent`+groundedness hit 裁判),`--n5`:系统 **1.00** / baseline **0.90**,macro **+10.0pp** 恰好达标。但 co2/thermal/lighting 两臂**全 1.00**,gap **全部来自 acoustic**(baseline 0.60,5 次里 2 次被 critic 正当否决,非 no_finish)。
- **根因**:单点**突发跳变**任务对 deepseek-v4-flash 基座太简单,朴素 ReAct 也能做对 → 架构优势(规划 DAG / Agentic RAG / 记忆)被任务难度**封顶**,对照趋同;acoustic `n=5` 的置信区间又宽(0.6±~0.44),这个 +10pp **易碎**,不是稳健结论。
- **(顺带)公平性修正**:baseline 跑前用 `arm(scenario)` 把模拟器读数对齐到 anomaly——否则 baseline `read_sensors` 读到 in-band 默认值、与自己 prompt 里声明的异常矛盾,会被**不公平地坑**(系统臂直接吃 anomaly、不读传感器)。那是噪声,不是架构差,必须消除才能归因干净。
- **状态**:⏳ 机制 + 公平性已坐实(这部分 ✅),但 ≥10pp 的**稳健**证据未立。下一步:扩 200 任务时**刻意塞判别性难任务**(多步规划 / 跨域副作用 / 复发 pattern 靠 memory / 误导性检索),而非堆更多简单单跳变;再换 GPT-4o 跨模型 baseline + GPT-4o+Claude 双裁判,去掉「自家 critic 当裁判」的主场嫌疑。
- **教训**:baseline 对照的**任务难度**决定信号强弱——基座够强时,简单任务上架构会趋同;要证架构价值,benchmark 必须含「基座单跑会栽、而规划/记忆/RAG 能救」的任务。看到「达标 ✓」先问**靠什么撑**:单域撑起的 macro 均值不能当结论。
- **关联**:`eval/runner.py` `Runner.compare`/`_print_compare_table` · `eval/ieq_bench/tasks/l3_e2e.jsonl` · `eval/reports/compare-20260531T150318Z.json` · EXECUTION_PLAN Phase 4「扩到 200」

### P-011 · Phase 4 · 自相矛盾陷阱实测不咬:强基座裸跑简单任务 ✅(负面结论)
- **现象**:为做 P-010 要的判别性难题,设计「自相矛盾陷阱」——高值 CO2(1600→≤1000、1500→≤800),赌 naive ReAct 会把 corpus ops-note 的「降 150-300ppm」复述进诊断正文、与自己的 target 矛盾、被 critic 正当否决。`--compare --only hard --n5` 实测:**两臂全 1.00 打平**,gap 0。
- **根因**:deepseek-v4-flash 基座**太强**。单跑 baseline 看输出:诊断「set ventilation to high... reduce CO2」、target=800/30min、**不复述任何降幅**,完全自洽。陷阱要 baseline「傻到复述」,但强基座不会。之前 generate flaky 是 SYSTEM 侧 temp>0 的随机现象(30%),不是 baseline 在 temp=0.2 下的稳定失败模式。
- **解决**:接受负面结论,不硬凑。两个 hard 任务**重标为 negative control**(证据:强基座能裸跑高值 CO2)。≥10pp-vs-ReAct 这条线**诚实判定:当前强基座 + 当前可做的简单任务上立不住**。作者拍板:误导性检索难题留 Phase 5 真 PDF corpus 落地后做(那时 ReAct 一次检索会中招、系统 grade/rewrite 能救),现在不勉强。
- **教训**:**对照实验里别把 baseline 想得太弱**。强基座会让「诱导 baseline 犯错」类陷阱失效——能拉开差距的是**结构性缺陷**(无记忆 / 无多步规划 / 无检索反思),不是「框得更难一点」。判别力来自 baseline **结构上做不到**的事(记忆消融 P-012),而非基座**偶尔会犯**的错。负面结论也是结论,如实记。
- **关联**:`eval/ieq_bench/tasks/l3_e2e_hard.jsonl`(negative control) · `eval/reports/compare-20260531T225509Z.json` · 引出 P-012 记忆消融

### P-012 · Phase 4 · 记忆消融:v3 prompt 对 resolved 案例「碰而不用」 ✅
- **现象**:新增 `--ablate-memory`(同 planner,召回 ON/OFF,看建筑特定知识有没有进 plan goal)。v3 首跑 4 任务只 1 个命中(macro lift **+0.25**):co2-damper(FAILED 案例)命中,thermal-solar / acoustic-fan / co2-allhands(resolved 案例)全没进 goal。
- **根因**:planner v3 的「用相似案例」段——对 **FAILED** 案例说「prefer a different angle」,逼 planner 点名坏路径(damper 因此进 goal);但对 **resolved** 案例只说「let it inform how you frame」,太弱,planner 把它当信心背书、goal 仍泛泛(solar/fan/all-hands 都没带出来)。记忆里的建筑特定原因没流进规划。
- **解决**:bump `planner/v4` 加规则「CARRY THE BUILDING'S SPECIFICS FORWARD」——召回案例(resolved 或 failed)若点出 anomaly 本身推不出的建筑特定原因/修法(卡死风阀 / 西晒 / 风机不平衡 / 固定时段超员),**必须在主 goal 里点名**;同时保留冷启动「(none)→ 别编造」护栏。**delta:macro lift +0.25→+0.75**(同环境)。第 4 个 acoustic 实际也带出了「HVAC supply fan for imbalance」,只是 gold 词 `rebalanc/unbalanc` 太死没匹配到「imbalance」→ 修 gold 为 `fan/imbalanc/...`(离线核对 off 不含 on 含,非造假)→ **+1.00**。L2-planner 种子 4/4 无冷启动回归。
- **教训**:① 记忆的价值是**消融轴**(同系统 on/off),不是 system-vs-ReAct——ReAct 无 planner 无记忆,拿它比会把架构和记忆混在一起。② prompt 里「参考一下」(advisory)和「点名写进输出」(imperative)效果天差地别;要某信息出现在产物里,必须**祈使**,不能「软建议」。③ gold 词要忠于模型**实际措辞**的变体,先看输出再定 gold(但只在确认 off 不含时放宽,不对着答案造假)。
- **关联**:`ops/prompts/planner/v4.md` · `agents/planner.py` `plan_with_recall` · `eval/runner.py` `ablate_memory` · `eval/ieq_bench/tasks/l3_recurrence.jsonl` · `eval/reports/ablate-memory-20260531T223417Z.json` · commit `207f2e6`

### P-013 · Phase 4 · 真 PDF corpus 接入(WELL v2 Air):知识形状变了 + pypdf 双层重复 + contextual prefix 上线  ✅
- **现象**:把 airquality 从手写占位(`sample_corpus` 12 段)换成真 WELL v2 PDF(IWBI 官方 CDN,366 页,取 Air concept p11-46)。三件事冒出来:① 真 corpus 的"知识形状"和占位**不一样**——占位假设核心是「CO2>1000ppm→加通风」,真 WELL Air 重点其实是 **PM2.5/PM10 阈值 + ventilation design + 过滤等级(MERV)**,CO2 真实阈值是 **500/750 ppm above outdoor**(A01),根本不是占位的 1000ppm;② pypdf `extract_text` 把标题/标签行**双输出**(行级:`Intent:\nIntent:`、`A01 AIR QUALITY | P` ×2);③ ingest 一直缺 node #10 contextual prefix。
- **根因**:① 占位是"安全的改写值"、凭直觉编的阈值/重点,和标准原文有系统性偏差——这是 P-010/P-011「判别性缺口」的另一面:连知识本身都被占位简化了;② WELL PDF 用描边/双层渲染,pypdf 逐 span 抽出就重复;③ 历史 TODO。
- **解决**:① `ingest.py` 加 `--source auto|corpus|sample` + `corpus_manifest.json`(文件→source/domain/页范围映射,零原文可提交;PDF 本体 gitignored);**渐进替换**——真 PDF 覆盖的 domain 换真、未覆盖留 sample filler(airquality 换真,thermal/lighting/acoustic 仍占位,不破其他域);② `collapse_repeated_lines` 折叠相邻相同行(111021 chars → 257 chunks,正文行不受影响);③ contextual prefix 接 node #10——`make_contextual_prefix` 走 router `ingest.contextual_prefix`→FAST(**deepseek-v4-flash**),prompt 版本化 `ops/prompts/ingest/contextual_prefix/v1.md`(不内联,硬约束 #4),document 前置复用 DeepSeek prefix cache,并发 8,**只改 `embed_text` 不改返回 `text`**(prefix 是检索信号非答案材料,沿用 retrieve.py 既有设计),失败退化为 ""(249/250 applied,1 失败不中断)。**delta**:retrieve airquality 召回从占位假值改为命中真实条款——CO2 query → `500/750 ppm above outdoor`、PM2.5 query → `MERV 14/16` 过滤表(reranker score 4.35),`.text` 纯净无 prefix 泄漏。
- **教训**:① **占位 corpus 不只是"内容少",是"知识被直觉污染了"**——真 corpus 一进来,下游 generate 引用的标准、critic 的物理范围、甚至 monitor 阈值都可能要跟着改(CO2 1000ppm 这种"常识"未必是标准原文)。这才是 Phase 4 ≥10pp 判别性的真正素材:真 corpus 的多来源/条件阈值会让 naive 一次检索中招、grade/rewrite 能救(P-011 留的那条线)。② build-time LLM(contextual prefix)放 ingest、不放 `mcp-rag-server`(硬约束 #9 约束的是 server,不是 ingest);失败必须退化不中断。③ PDF 抽取的工程噪声(双层重复)要在入库前清,否则污染 BM25/embedding。
- **关联**:`rag/ingest.py`(`collapse_repeated_lines`/`make_contextual_prefix`/`apply_contextual_prefixes`/`select_passages`)· `rag/corpus_manifest.json` · `ops/prompts/ingest/contextual_prefix/v1.md` · `core/router.py` node #10 `ingest.contextual_prefix`→FAST · `rag/corpus/well-v2.pdf`(gitignored)· `ops/llm_routing.md` #10

### P-014 · Phase 4 · 真 corpus 切换回归:monitor 的「ASHRAE 62.1 = 1000ppm」是占位误区,对齐到 WELL 900  ✅(bench 种子重写延后)
- **现象**:P-013 真 WELL corpus 接入后做切换回归(demo `co2_spike` + airquality bench 复跑),三层打架浮出:① **检索 bench 1.00→0.60**——`L1-retrieval-co2-001`(gold=`ashrae-62.1`)、`L1-retrieval-co2-ops-001`(gold=`ops-note`)的 gold source 是旧 `sample_corpus` 的标签,真库里只有 `well-v2`,必失败;② **monitor**:`thresholds.py` co2 rule 写「co2 ≤1000 ppm(ASHRAE 62.1 indoor air-quality guideline)」;③ **generate**:诊断输出「exceeds the ASHRAE 62.1 limit of 1000 ppm (excerpt 1) … reduce below 800 ppm」。
- **根因**:① **「ASHRAE 62.1 设 1000ppm CO2 上限」本身是广为流传的误区**——62.1 设的是通风换气率,从不设 1000ppm 的 CO2 健康上限(那条常被误传);真 WELL v2 A01 是 **900 ppm(1-point)/ 750 ppm(2-point)**,且「1000」在 WELL corpus 里 **0 命中**(`ASHRAE 62.1` 在 corpus 里 7 命中,但全是「通风率按 62.1 Natural Ventilation Procedure」,不是 CO2 上限)。② 这条假归因从 `thresholds.py` 的 `rule_violated` 串一路传进 monitor anomaly → planner subtask goal → generate,generate 就把 goal 的「1000/ASHRAE」当成「excerpt 1」引用了(典型的**占位知识污染下游归因**,正是 P-013 教训①预言的)。③ bench 种子在占位时代写死了 `ashrae-62.1`/`ops-note` 这些只存在于 `sample_corpus` 的 source 标签。
- **解决**:**作者拍板「触发线降到 900 对齐 WELL」**——`thresholds.py` co2 `high 1000→900`、rule 改「co2 must stay <= 900 ppm (WELL v2 Air, 1-point threshold)」,去掉 ASHRAE 假归因。连带排查:`RoomState.co2_ppm` 默认 650(动态稳态 ~720)均 < 900,非 co2 场景不会被新阈值误触发;critic 物理范围 `(350,5000)` 含 900/750 无打架;co2_spike(1300)仍触发、降到 679 仍 met。**复跑 demo `co2_spike` 实证对齐**:anomaly→「≤900 ppm」、goal→「against the 900 ppm threshold」、诊断→「exceeds the **900 ppm threshold specified in the WELL v2 standard (excerpt 1)**」(excerpt 1 真含该阈值,误归因消失)、`target_value=900`、critic approve、verifier met/closed。**bench 种子重写延后**(作者拍板 Q2):两个失效检索种子(`ashrae-62.1`/`ops-note` gold)+ generate/e2e 种子里硬编码的「1000/ASHRAE」anomaly 输入,统一留到 Phase 4「扩 200 判别性难题」时按 WELL 重写;短期 retrieval bench 维持 0.60(known-fail,非检索能力退化,是 gold 标签陈旧)。
- **教训**:① **占位阈值可能编码了一个「常识级误区」**——不是"数值不精确",是"把一个流行的错误说法当标准写进了系统真值";真 corpus 是照妖镜,一进来就把它逼出来。这比 P-013 的「知识形状不同」更尖锐:连 monitor 这种最底层的确定性规则都得跟着真标准改。② 切 corpus 不是只换 `rag/`,是一条 **corpus→monitor 阈值→planner goal→generate 引用→bench gold** 的纵向回归链,每一环都可能埋着旧占位的影子,必须端到端复跑才照得全。③ bench 的 gold 也是"占位资产",换真 corpus 后 gold 同样要迁移,否则绿表变红还以为是能力退化。
- **关联**:`sensing/thresholds.py` co2 行 · `agents/monitor.py`(`rules_text` 渲染)· `eval/ieq_bench/tasks/l1_retrieval.jsonl`(co2-001/co2-ops-001 known-fail,待 200 重写)· `eval/ieq_bench/tasks/{l2_generate,l3_e2e}.jsonl`(硬编码 1000/ASHRAE,待 200 重写)· P-013（真 corpus 接入,本条是其下游回归）

### P-015 · Phase 4 · runner 并行化:`_pmap` 暴露 reranker 并发不安全(transformers 5.9.0)+ seed 路径漏传 workers  ✅
- **现象**:给 `eval/runner.py` 加 `_pmap`(ThreadPoolExecutor)并行 generate/compare 的内层 sample 循环后,bench 一跑就崩:`RuntimeError: expected scalar type Half but found Float`(reranker `layer_norm`)+ 偶发 `ImportError: cannot import name 'AutoModelForSequenceClassification' from 'transformers'`。但**单次/顺序** retrieve 完全正常(直接 `RetrievalStack.retrieve` / 单 `call_tool` / 单 subgraph invoke 都 OK)。还发现 `--workers 1` 不生效:线程名探针显示样本跑在 `ThreadPoolExecutor-3_*`、`self.workers` 恒为 4。
- **根因**:① **transformers 5.9.0**(06-06 随 pypdf 接入连带 bump)下 bge-reranker **不是并发安全的**——多线程并发 forward 让 fp16 模型 dtype 状态错乱(Half/Float 混用),并发首次 `_get_stack` 又多线程同时触发 lazy import `AutoModelForSequenceClassification` 竞态拿到半初始化模块。改前 generate bench 是**顺序 for 循环、无并发**,所以一直没暴露;我的 `_pmap` 首次引入并发 retrieve 才点着这个雷。② `Runner` 构造在 seed/cap 路径(`main()` 末尾,原 line 743)**漏传 `workers=args.workers`**、一直用默认 4——`replace_all` 漏改的发散调用点,导致 `--workers` 在该路径被忽略(所有"workers=1"测试其实都在 4 路并发,误导排查良久)。
- **解决**:① `rag/retrieve.py` 加 `self._gpu_lock`,串行化 `_embed`+`_rerank_scores` 的 GPU forward(~50ms,远低于真正并行的 cloud-LLM 成本,代价可忽略);② `mcp_servers/rag/server.py` 的 `_get_stack` **双检锁**,并发首访只构造一次栈(import 单线程化);③ runner 加 `_warm_rag` 预热,线程 fan-out 前单线程把栈加载好;④ 补 seed 路径的 `workers=args.workers`。**验证**(同环境 `--n6` 同任务):workers=1 → decompose 间隔 ~17s 真顺序 / `WALL=162.7s`;workers=6 → decompose 0.24s 内并发 / `WALL=48.7s`,**3.3x 提速**,两者 consistency 全 1.00、零 dtype/import 错。
- **教训**:① **共享 GPU 模型单例不是自动线程安全的**——并行化调用方必须显式串行化 forward(锁)或每线程独立实例;这不只是 bench 的事,真实 `mcp-rag-server` 上线接并发 MCP 请求同样需要,属前瞻修复而非临时补丁。② transformers 5.x 把 `torch_dtype` 废弃为 `dtype`,并发下旧参数 dtype 处理更脆;锁顺带规避(顺序路径仍工作,故未强改加载参数,留最小改动面)。③ `replace_all` 会漏发散调用点(`Runner()` vs `Runner(n_samples=...)` vs 末尾那个无参版),改完务必 `grep "Runner("` 核对全部构造点。④ **工具坑自记**:`pkill -f "eval.runner"` 会连带杀掉命令行里含该串的当前 shell 自身;前台 `sleep` 被 harness 禁——排查期间两者各坑掉若干次 run,误判为代码问题。
- **关联**:`eval/runner.py`(`_pmap`/`_warm_rag`/`--workers`/seed 路径 `workers`)· `rag/retrieve.py`(`_gpu_lock`)· `mcp_servers/rag/server.py`(`_stack_lock` 双检)· transformers 5.9.0 · P-002(bge-m3 HF 在线检查拖慢加载,排查期每次 run 多付 ~25s)

### P-016 · Phase 4 · thermal/lighting/acoustic 真 corpus 一次性接入(WELL v2 同一 PDF 三 concept)+ 占位阈值待对齐  🔧(接入完成,下游对齐留下次)
- **现象/动作**:原计划 acoustic 用 WHO Noise、thermal/lighting 卡在付费 ASHRAE 55 / EN 12464。复盘发现**手上那份 airquality 用的 WELL v2 PDF(366pp)本就是 10-concept 全集**——Light / Thermal Comfort / Sound 三个 concept 都在同一份里,免费、零额外获取。作者据此拍板弃付费标准。pypdf 扫描定位全 10 concept overview 物理页(Air11 / Water47 / Nourishment72 / **Light103** / Movement130 / **Thermal160** / **Sound185** / Materials214 / Mind247 / Community274),`corpus_manifest.json` 加三条(同一 `well-v2.pdf`,靠 `pages`+domain filter 切片:lighting 103-129 / thermal 160-184 / acoustic 185-213),`ingest --source corpus`(**按作者指定三新域先不做 contextual**)→ **824 chunks**,四 domain 全真、sample 占位全退场。三域检索 sanity-check 各命中 WELL 真值。
- **根因(为何之前以为要找付费源)**:把"airquality corpus = WELL Air"记成了"WELL 只有 Air",忽略同一 PDF 已覆盖另三域;manifest 当初注释还特意写"p47 onward excluded 以免误标 airquality",强化了"其他域没资料"的错觉。其实只是当时按 domain 渐进接入、没回头看整本覆盖面。
- **检索证实的占位 vs WELL 真值差异(下次对齐素材)**:

  | domain | `thresholds.py` 占位 | WELL v2 真值(检索命中) |
  |---|---|---|
  | thermal | 19-26 °C | dry-bulb **21-25 °C**(≥90% 占用时段)+ 风速 ≤0.2 m/s + 上限 33.5 °C,引 **ASHRAE 55-2013** |
  | lighting | lux ≥300,来源标 **EN 12464-1** | 引的是 **CIBSE SLL Code for Lighting**(非 EN!);循环/储藏/餐饮区 **110 lux(10 fc)**;work area 按 task+age 分级(Option 2 predetermined) |
  | acoustic | ≤55 dBA(单值) | **分级 50/55/60 dBA**(Cat 1/2/3)+ dBC 70/75/80;**卧室 ≤35 dBA**(Leq 夜间 12h);5 min 均值 |

- **教训**:① **找新数据源前先把手上资料的覆盖面查全**——一份 WELL v2 省掉三处付费墙,差点白找 WHO/ASHRAE/EN。② **再次印证 P-013/P-014**:lighting 占位把来源标成 `EN 12464-1`,但真库里引的是 **CIBSE SLL** —— 又一个"占位引用了不在 corpus 里的标准"的雷(generate 会引 EN 但库里只有 WELL/CIBSE,同 P-014 的 ASHRAE-1000 同型),三域 thresholds 都得走 P-014 的 corpus→monitor→planner→generate→bench 纵向对齐链。③ 三新域**先不做 contextual**(embed_text=text),与 airquality(已接 #10)暂不对称;补做需 `--contextual` 全量重 embed 四域,留下次连同 thresholds 对齐一起。
- **下次接(清单)**:1) `thermal` 19-26→**21-25 °C**(WELL T01),去 ASHRAE 误标;2) `lighting` 来源 `EN 12464-1`→**CIBSE SLL/WELL**,用更精确 query 复核 work area 具体 lux 值;3) `acoustic` 单值 55→选 **WELL 档位(Cat)** 或加 dBA 分级;4) 三域 thresholds 改完跑 P-014 纵向链端到端复跑(demo + monitor rule + generate 引用);5) 三域各加 bench 种子(retrieval/generate/e2e);6) (可选)三域补 contextual 全量重 embed。
- **关联**:`rag/corpus_manifest.json`(+3 条)· `rag/ingest.py`(`--source corpus`,824 chunks)· `sensing/thresholds.py`(thermal/lux/noise_db 三行待对齐)· `rag/retrieve.py`(验证)· P-013/P-014(同型纵向对齐)· memory `project_corpus_source`

### P-017 · Phase 5 · 问答管家:从"全拉"修正为"意图分类按需取数" + 范围判定免 RAG  ✅
- **现象/动作**:做问答优先 MVP 的 `ConversationalAgent` 时,初版设计是"把所有廉价源(读数/历史/incident/facts/SOP)全拉进 LLM 一次合成"。作者两次质疑:① "不管问什么都把四个领域的历史全传给 LLM 吗?" ② "温度在正常范围内吗,要去调 RAG 了?"
- **根因**:① 全拉图省事,但违背 CLAUDE.md 写的 memory-first **dispatch**(本意就是按需取数);数据增长后(incident/facts 累积、读数序列上万行)token 爆 + 噪声干扰抓重点;对"现在温度多少"这种简单问题灌一周 incident 纯浪费。② 一度以为"判断是否正常"要查标准 corpus。
- **解决**:① 改**两步式**——一次轻量 flash 出 `RetrievalPlan`(need_stats/incidents/facts/sops + domain/sensor/since),**基线(read_sensors+thresholds)恒带、大源按计划选择性拉 + 领域过滤**;再一次 flash 合成。实测按需取数生效:"现在温度多少"/"温度正常吗" sources=[](只基线)、"过去24h CO2" sources=['stats']、"最近异常" sources=['incidents']。② **范围判定免 RAG**:"在不在范围/超标没/合不合格" = 当前读数 vs `thresholds.py`——它是 corpus 阈值的**蒸馏固化**、且是 monitor 判异常的**同款真值**(管家用它答与系统自洽);只有"标准条款原文/出处/为什么是这个值/跨标准差异"才需查 corpus(留后)。故 MVP **不上 RAG/embedding**,免 key、RPi 零本地模型(不背 torch)。两次 LLM 都 deepseek-v4-flash(作者拍板对话全 flash、不升 V3)。
- **教训**:① "廉价源"也不该无脑全拉——数据会增长,dispatch 的价值就是**按需**;要某信息进上下文,用意图分类显式 gate,别图省事全灌。② 判断"要不要 RAG"看**要不要标准原文**:数值阈值比较有蒸馏值(thresholds)就够,thresholds 本就是 corpus 的固化,且与 monitor 共享真值=自洽——这也复用了 P-014"thresholds 是系统真值"的结论。③ FastAPI 的 `File(default=None)` 放参数默认里会被 ruff B008 拦,改 `UploadFile | None = None`(FastAPI 仍识别为上传)即可。
- **关联**:`agents/conversational.py`(两步式 + `RetrievalPlan`)· `ops/prompts/conversational/{dispatch,respond}/v1.md` · `sensing/history.py`/`ops/sampler.py`/`sensor_readings` 表 · `core/router.py` node `conversational.dispatch`=FAST · `sensing/thresholds.py`(范围判定真值)· `voice/provider.py`(级联 mock)· `frontend/`(FastAPI+HTMX+Web Speech)· 计划 `5-incident-...-resilient-duckling.md`

### P-018 · Phase 5 · 问答管家多轮 + 流式:前端持有 history(后端无状态)+ mypy 流式 overload 退化  ✅
- **现象/动作**:把单轮无状态问答升级为**多轮 + 流式**(作者拍板英文对话)。
- **关键设计**:① **多轮无状态**——history 由**前端持有**、每轮随请求带上,后端**不存 session**(免 session 表/内存 dict、FastAPI 多 worker 不怕状态不共享),后端 `_parse_history` 防御式解析 + 滑窗截最近 6 条;**dispatch 和 respond 都要喂历史**(只喂 respond 不够:dispatch 拿不到上文就把裸追问 "what about humidity?" 的 sensor 分类不出来)。prompt bump **v2**:从「`$query` 占位塞进单 user message」改为 **system 指令 + 历史多 message + 当前 user(含 `<context>` 包当轮取数)**,全英文。② **流式**——`router.stream()` yield delta,**fallback 只在「还没吐出任何 token」前切换**(`produced` flag);一旦开吐,中途失败直接抛(重试会重复输出)。`respond_stream` 是 generator,**dispatch+gather 惰性到首次迭代才跑**(StreamingResponse 一迭代就触发,首 token 含这段)。语音路径保留**非流式** `respond`(浏览器 TTS 要完整文本)。
- **踩坑**:`self._client.chat.completions.create(stream=True, **kwargs)` 里 `**kwargs: Any` 让 openai `create` 的 **overload 解析退化**,mypy 把流式返回推断成 `tuple[str,Any] | ChatCompletionChunk`,迭代 `chunk.choices` 报 union-attr。修:`cast("Iterator[ChatCompletionChunk]", create(...))` 钉死类型(非流式 `complete()` 无此问题——overload 在 stream 缺省时正确选 ChatCompletion)。
- **实测**(真 LLM,Postgres/Qdrant 起):"is the temperature ok?"→sources=[]、流式 33 块、首 token 1.9s、答 22.5℃∈19-26;裸追问 "what about humidity?"(未点名环境湿度)→turns=1、**接住上轮**、答 45%RH∈30-60、还呼应上轮说 "also fine"。ruff + mypy(core 严格)全绿。
- **教训**:① 多轮的「循环」和「上下文」是两件事——Web 形态循环天然有(每条消息一轮),真正要补的是**把历史喂进去**;且**两步式里每一步**(分类+合成)都要吃历史,漏掉分类那步指代就断。② history **前端持有 + 后端无状态**是 MVP 最省的多轮方案(无 session 管理),代价是每轮多传一点 + 后端要兜底解析/滑窗。③ 流式 fallback 不能像非流式那样无脑重试——**已吐字就不能重来**,只能在未产出前切换。④ `**kwargs: Any` 会击穿 SDK 的 `@overload`,流式/非流式返回类型混淆,`cast` 比一堆 `type: ignore` 干净。
- **关联**:`core/router.py` `stream()`(`produced` fallback + cast)· `agents/conversational.py`(`respond`/`respond_stream`/`_prepare`/history-aware `_dispatch`)· `ops/prompts/conversational/{dispatch,respond}/v2.md` · `frontend/api/main.py`(`/api/chat` StreamingResponse + `_parse_history` 滑窗)· `frontend/app/index.html`(ReadableStream 流读 + 客户端 history + en-US)· P-017(本条是其多轮+流式续作)

### P-019 · Phase 4/5 · 三域 thresholds 纵向对齐:走完 P-016 留的 corpus→monitor→planner→generate→bench 链 + 三裁断  ✅
- **动作**:补完 P-016「下次接」清单 1-4——thermal/lighting/acoustic 三域 `thresholds.py` 占位对齐到真 WELL corpus,并端到端走完 P-014 定义的 **corpus→monitor 阈值→planner goal→generate 引用→bench gold** 纵向链。
- **取证先行(P-014 方法论)**:不信二手转述,直接 scroll Qdrant `ieq_standards` 三域 chunk 提真值。三处占位病灶 + **推翻一个二手说法**:thermal **19-26→21-25 °C**(WELL T01 干球 "70-77°F(21-25°C)",引 ASHRAE 55-2013);lighting **300→320 lux**(WELL Predetermined 表 "Offices and classrooms: 30 fc(320 lux) at task surface",循环区 110)——corpus **5 个参考指南都列了**(IES/EN 12464-1/ISO 8995/GB50034/CIBSE SLL),P-016 快照「EN 12464 不在库」**不准确**,真问题是数值 300→320;acoustic **55→50 dBA**(WELL S02 表 Average Leq 1-point/Cat3 = 50,占位 55 比最低合规线还松)。RoomState 默认(22.5/420/41)排查全仍 in-band、critic 物理范围不打架。
- **运行链对齐**:`thresholds.py` 三行值+rule(长串括号隐式拼接过 E501,**拼接结果脚本校验逐字 == bench rule_violated**)、`room.py` band 注释、`scenarios.py` 三 demo 描述;monitor `rules_text()` 自动渲染对齐。
- **bench 迁移 + 三裁断**:① `l1_retrieval` 5 条 gold_sources(含 co2 残留 ashrae-62.1/ops-note)→`well-v2`——但**诚实标注退化**:真 corpus 单 source,`score_retrieval` 按 source 比对会让 hit@k **恒真**,迁移=消除 known-fail 但变烟雾测试,真判别留扩 200(同理 `l2_rewrite` 单 source MRR 恒 1 失效,**不动**)。② 运行链镜像 jsonl(planner/recurrence/e2e/generate/e2e_hard)`rule_violated` 统一成新 thresholds 串(脚本只替含旧占位标志的,**dry-run 抓出并放过** `hardwell` 的 `800 ppm WELL DCV`——故意的更严负控,非残留)。③ **`l2_critic` 整组不动**——封闭确定性单元,`co2-good` 诊断 target=1000 配旧 rule 才自洽,改 anomaly rule 而不动 target 会**直接破坏 gold approved**;critic 的 anomaly.rule 非运行链镜像。④ gold_values 据**实跑**定(下条)。
- **实跑三域链环④(真 LLM + GPU 检索,demo 三场景)**:generate 全部 grounded 到真 corpus 真值、有 excerpt 出处、无假归因——lighting "320 lux ... per **CIBSE SLL** (excerpt 1)"(故 e2e gold `EN 12464`→**`320`**,锚数值不锚随检索波动的标准名);thermal "29°C ... 21-25°C comfort range, **ASHRAE 55-2013** (excerpt 2)";acoustic "**Category 3 background noise limit of 50 dBA (Leq)** (excerpt 5)"。co2 据 P-014 已证引 WELL/900,gold `ASHRAE`→`WELL`。
- **正交既有问题(非本次引入,记待跟进)**:acoustic demo **critic 否决**——理由 `target_time_min=15` 不现实("降噪靠设备选型+立面改造,15min 不可达"),与 50/55 阈值无关,是 acoustic specialist 的 expected_outcome 时间窗问题(降噪本就非 15min 物理过程)。诊断真值已对、critic 在正确工作。留后:acoustic `target_time_min` 调成合理窗 / 或 demo 场景标注。
- **校验**:ruff+format+mypy(strict core)全绿;9 jsonl 全合法;残留扫描仅剩 `l2_critic`(故意保留)。
- **教训**:① **真 corpus 是照妖镜,二手转述也得复验**——这次连「EN 12464 不在库」的快照说法都被取证推翻(它在,5 选 1),靠 scroll 真 chunk 而非信记录。② bench gold 迁移**不是无脑替换**:单 source 退化(retrieval 假绿)、封闭单元(critic 改 rule 破坏自洽)、故意负控(hardwell)三类都要 dry-run + 人工裁断,脚本「按 sensor 统一」会误伤。③ 纵向对齐**必须实跑链环④**才算闭环——只改 thresholds 不跑 generate,看不出它引 CIBSE 还是 EN、引 320 还是编值。④ acoustic 的 critic 否决证明 **plausibility/coherence 闸在干活**,也暴露 specialist 时间窗与物理过程脱节——真闭环系统该有的张力。
- **关联**:`sensing/thresholds.py`(三域值+rule)· `sensing/simulator/{room,scenarios}.py` · `eval/ieq_bench/tasks/{l1_retrieval,l2_planner,l2_generate,l3_e2e,l3_e2e_hard,l3_recurrence}.jsonl`(迁移)· `l2_critic.jsonl`(故意不动)· `eval/metrics.py`(score_retrieval/generate_hit 判分语义)· P-014(co2 同型,本条是三域版)· P-016(补完其「下次接」1-4)· memory `project_corpus_source`

### P-020 · Phase 4/5 · critic 否决在「无 actuator 的人工域」升级为请人介入,而非 FAILED  ✅
- **动作**:作者拍板「声学只报 incident 然后请人类介入」。承接 P-019——acoustic 的 critic 否决(masking/换设备不降 dBA)本是**正确**判断,但归宿是 FAILED(像出错),不符合「系统做不到 → 交人」的语义。把 critic 否决在 Tier3(无 actuator、必然人工)域**升级为走 autonomy_gate 请人**;Tier1/2(会自动执行)域仍 FAILED。
- **关键洞察**:critic 否决的含义是「别让系统自动执行这个方案」。对**有执行器**的域(airquality Tier1)否决 = 别自动执行 = FAILED 合理;对**无执行器、本就人工**的域(thermal/lighting/acoustic Tier3)否决 = 系统自己做不到 = **这正是该请人的信号**,不该当失败。**critic 的判断标准一字未动**(没放松),只改否决之后的去向。
- **实现(分层干净)**:① `_TIER_BY_DOMAIN` + `tier_for_sensor()` 从 `core/graph.py` 移到 `core/state.py`——critic 要按 tier 分流但**不能 import graph(循环依赖)**,放 state 两边共用。② `_route_after_critic`:approved 或 (disapproved + Tier3) → `autonomy_gate`;否则 END。③ `critic._disapprove`(新):Tier3 → status `AWAITING_APPROVAL` + ticket patch(不 FAILED);Tier1/2 → 原 `_fail`(FAILED)。④ `autonomy_gate` interrupt payload 带 critic `concerns`——escalation 时 reason 区分「No autonomous fix available」,把 critic 顾虑交给人研判。**复用现成的 `AWAITING_APPROVAL` 状态 + Tier3 interrupt 机制,不新增状态、不动 schema、不绕过 autonomy_gate(硬约束#2)**。
- **实测**:acoustic `noisy_room` → `critic_disapproved`(masking 加噪不降 dBA)→ `critic_escalate_human` → ticket `awaiting_approval` → ⏸ Tier3 interrupt(reason「No autonomous fix available」+ concerns 带 masking 顾虑)。**不再 FAILED**。回归:thermal `overheating` approved → Tier3 interrupt(concerns=[],普通审批,payload 改动没崩);co2 Tier1(AUTO)不进 interrupt 分支,approved 路径未触。ruff+mypy(strict core)全绿。
- **教训**:① 同一个「否决」对不同 tier 该有不同归宿——**有自主手段的域否决=失败(别瞎执行),无自主手段的域否决=升级人工(本就该人来)**。这比「放松 critic 兜底」健康得多:质量闸不破、坏方案不漏、人只在系统真做不到时被叫。② domain→tier 映射放 state(非 graph)是为让 critic 读它而不循环 import——**路由策略该放在 agent 和 graph 都够得着的中性层**。③ mypy 收窄:`escalated = verdict is not None and ...` 这种 bool 中转变量 mypy 不借它 narrow,要把 `verdict is not None` 直接写进使用处的 if。
- **边界/待跟进**:acoustic 无 actuator(Phase 5),人 approve 后 `action` 节点暂无 acoustic 执行器(demo 停在 interrupt 符合「请人介入」语义);Tier1/2 否决 → replan 仍 Phase 3 未接(否决=END,无重规划)。
- **关联**:`core/state.py`(`_TIER_BY_DOMAIN`/`tier_for_sensor`/复用 `IncidentStatus.AWAITING_APPROVAL`)· `core/graph.py`(`_route_after_critic`/`autonomy_gate`)· `agents/critic.py`(`_disapprove`)· P-019(承接其 acoustic 时间窗发现 + v5 回滚)· CLAUDE.md 硬约束#2

### P-021 · Phase 5 · 真语音级联:阿里云百炼 CosyVoice 流式 TTS + 英文 STT,push 回调桥成拉取流 + NDJSON 多路复用  🔧(mock 链路 E2E 验通,真 key 段待填)
- **动作**:承接 P-018 文本流式,把语音从 `MockVoiceProvider`(浏览器 Web Speech)升级到**真云级联**——作者拍板 STT/TTS 走**阿里云百炼**,TTS=**CosyVoice `cosyvoice-v3-flash`**(实时最快档),STT=`paraformer-realtime-v2`;**butler 全英文**(STT en + 英文音色 + LLM 已英文)。
- **延迟设计(作者核心诉求「LLM 既然流式,怎么让对话延迟最低」)**:**不等整段答完再合成**——`respond_stream` 的 token 边吐边灌进 CosyVoice 双工 WS(`server_commit` 服务端自动分句),`audio.delta` 边回边播;出声延迟 ≈ STT + LLM 首句 + TTS 首包,**而非各段相加**。LLM 首 token ~1.9s(P-018)是地板,把 TTS 藏在生成背后而非叠加其后;压延迟手段:预热/常驻 WS 连接(首包含建连)、prompt「先一句话直答」、PCM 24k 免解码、选 v3-flash。
- **关键实现**:① **push→pull 桥**——DashScope SDK 是回调推送(`ResultCallback.on_data(bytes)`),但要 `synthesize_stream(text_iter)->Iterator[bytes]` 拉取式:`queue.Queue` + feeder 线程(迭代 token `streaming_call()` 喂字、`streaming_complete()` 收尾)、回调把音频帧 put 进队、主生成器 get 出来 yield、`_DONE` 哨兵收尾,完后读 `get_first_package_delay()`。② **NDJSON 多路复用** `/api/voice/stream`——一条流里既要**屏幕文字**又要**语音**:`_tee()` 把 token 流分两路(put 进 bus 给前端显示 + yield 给 TTS),audio 帧 base64 进同一 bus,主生成器序列化成 `{query|token|audio|done}` 行。③ **前端**——`PcmPlayer`(Web Audio 把 24k PCM 帧排到游标 gapless 播)+ `WavRecorder`(ScriptProcessor 抓音频、降采 16k、自编 WAV;MediaRecorder 出 webm/opus 不合 paraformer)+ NDJSON 流读。④ **降级**——`get_voice_provider()` 无 key/构造失败退回 mock;mock `synthesize_stream` drain token 不出音频,前端无 audio 帧时回退浏览器 `speak()`,链路恒可演示。
- **实测(mock 链路,真 ConversationalAgent+DeepSeek)**:`TestClient` 打 `/api/voice/stream`「what's the CO2 right now?」→ HTTP 200、`query`1+`token`31+`done`1、**无 audio**(mock 正确)、`first_package_ms=None`、答 "650 ppm ... at or below 900 ppm"(grounded 英文)。tee→线程→bus→NDJSON 整条编排验通。ruff+mypy(strict core)全绿。
- **Δ 同 session 改型(作者实测后定)**:作者测出**新加坡 endpoint 比北京快 ~5s**,拍板改用 **Qwen3-TTS-Flash**——TTS 从 CosyVoice 换成 **`qwen3-tts-flash-realtime`**(新加坡 `wss://dashscope-intl.aliyuncs.com/api-ws/v1/realtime`),音色 **`Jennifer`(詹妮弗/美剧大女主)** + `language_type=English`。**接口换家族**(CosyVoice `tts_v2.SpeechSynthesizer` ≠ Qwen):`QwenTtsRealtime.connect()/update_session(voice,language_type)/append_text(chunk)/finish()` + 回调 `on_event` 取 `response.audio.delta`(base64,**仍 PCM 24k → 前端 PcmPlayer 不用动**),首包用内置 `get_first_audio_delay()`;完成检测用**终止事件集 + 静默超时双保险**(realtime 终止事件名不定,避免挂死)。`CosyVoiceProvider`→`DashScopeVoiceProvider`(名副其实)。
- **STT 新加坡区踩坑链(真 key 逐个排掉)**:① `paraformer-realtime-v2`(`Recognition`/`/api-ws/v1/inference`)→ **401**:`Recognition` 用全局 `base_websocket_api_url`(默认北京),新加坡 key 打北京 ws 被拒;② ws 指新加坡 → **status 44 Model not found**:**paraformer 新加坡区没有**,intl 的 ASR 是 **Qwen3-ASR** 系;③ OpenAI 兼容 `audio.transcriptions` → **404 不支持**;④ 改 **`qwen3-asr-flash` 走 `MultiModalConversation.call`**(HTTP,audio 走 `messages` 的 `file://` URL,`asr_options={'language':'en','enable_lid':False}`)→ **200 通**。STT 不再走 ws,`dashscope_asr_ws_url` 删除。与作者拍板 `qwen3-asr-flash` 一致。
- **两端真实 E2E 全绿(新加坡 key)**:TTS round-trip 首包 **666/684ms**;TTS→降采16k→STT 保真("...650 ppm..."→"CO two level of six hundred and fifty parts per million...",数字/缩写口语化正常);**真端点 `/api/voice/stream` 上传 wav 全链路**:STT→LLM 流式 34 token→**真 TTS 28 音频帧**→done,首包 **651ms**,grounded 英文。**`.env` 需 `VOICE_PROVIDER=aliyun`**(只填 key 不够,否则端点退回 mock→transcribe 空→error,已实测踩到)。mock 链路改名后复验仍通。
- **待跟进**:真音频段卡 `DASHSCOPE_API_KEY`(未在 `.env`)——`ops/scripts/voice_smoke.py` 填 key 一命令验真合成+量首包;STT 二期上流式;realtime `commit`/server 分段模式 + WS 预热常驻连接留优化。voice/ 的 dashscope `import-untyped` 仅因第三方缺 stub(严格门只管 core)。
- **教训**:① 回调推送 SDK 接流式 Web,**queue+线程桥成拉取迭代器**是干净解法——别在回调里直接 await/yield。② 一条 HTTP 流要同时带文字和二进制,**NDJSON + base64 帧**比双连接简单;`_tee` 让 token 一源两用(显示 + 喂 TTS)。③ 浏览器要真 wav 给云 STT 得**自己编 WAV**(MediaRecorder 给的是 webm/opus);TTS 回放用 Web Audio 排游标做 gapless,别每帧 new 一个 `<audio>`。
- **关联**:`voice/provider.py`(`DashScopeVoiceProvider`/`synthesize_stream` push-pull 桥 + QwenTtsRealtime/`get_voice_provider` 分支)· `frontend/api/main.py`(`/api/voice/stream` NDJSON 多路)· `frontend/app/index.html`(`PcmPlayer`/`WavRecorder`/NDJSON 流读)· `core/config.py`(`dashscope_api_key`/`dashscope_ws_url`/`dashscope_http_url`/`tts_model`/`tts_voice`/`tts_language`/`asr_model`)· `ops/scripts/voice_smoke.py` · P-018(文本流式前作)· memory `project_voice_cascade`/`project_butler_english` · 计划 `5-agent-mrk1010-...-lamport.md` Task 1

### P-022 · Phase 5 · 树莓派部署 bring-up:WSL→Pi 网络客户端隔离 + ARM torch cu121 + transformers 版本 + sed 误删  ✅(env 就绪,Qdrant/ingest/spike 待续)
- **背景**:展台实物部署启动,从 WSL 远程 bring-up Pi4(Debian 13 Lite / aarch64 / 8GB)。一口气踩了四个坑,逐个记下。
- **坑1 · WSL 连不上 Pi(网络层)**:WSL(192.168.1.120)与 Pi(192.168.1.152)**同子网**,但 IPv4 ping 100% loss、ARP 建不起来;IPv6 补 on-link /64 路由后从 "Network unreachable" 变 "No route to host"(NDP 邻居不可达)。**根因 = 路由器开了无线客户端隔离(AP isolation)**:同 AP 下客户端二层互不可见(ARP/NDP 被拦),但都能和网关通——所以 Windows 经网关三层路由的 IPv6 能连 Pi,而 WSL 的 on-link 直连不行(Windows IPv4 同样因 on-link 被隔离,只是没试)。**解**:Windows `netsh interface portproxy add v4tov6 listenport=2222 → Pi 稳定 EUI-64 IPv6:22`,WSL 经 `ssh pi`(127.0.0.1:2222 别名,公钥趁 Pi 本地会话装)免密连。**教训**:同子网 ping 不通 + 双向 ARP 空 = 强烈指向客户端隔离;别死磕直连,**复用「已验证能连的那条路」(Windows→Pi IPv6)做转发**最省。根治可关路由器隔离(IPv4 直连)。
- **坑2 · torch cu121 wheel 装不上 aarch64**:`pyproject.toml` 用 `[tool.uv.sources] torch = {index="pytorch-cu121"}`(给开发机 RTX3060),Pi aarch64 在 cu121 index 没 wheel。**解**:Pi 维护一份**去 cu121 的 pyproject 变体**(aarch64 的 PyPI 默认 torch wheel 本就是 CPU 版),删 uv.lock 重解析 → torch 2.6.0+cpu。**注意:后续从开发机 rsync 必须排除 `pyproject.toml`/`uv.lock`**,否则被 cu121 版覆盖。
- **坑3 · transformers 版本漂移**:Pi 重解析装了最新 transformers 5.10.2,`from transformers import PreTrainedModel` 崩(与 sentence-transformers 5.5.1 不兼容)。**解**:pin 到开发机版本 transformers **5.9.0** / st **5.5.1**(纯 python,跨平台一致)。**教训**:删 lock 重解析会拉最新版引入不兼容;关键库要 pin 到开发机已验证的版本。
- **坑4 · sed 范围误删半个 pyproject**:`sed '/tool.uv.sources/,/explicit = true/d'` 把 pyproject 从 93 行删到 54 行——因为 **torch 依赖行的注释里有 `(see [tool.uv.sources])` 字样**,被起始模式误匹配,从 torch 行就开始删,连带删了所有依赖。**解**:改用「本地写好整份正确文件 → rsync 覆盖」的可控法。**教训**:`sed` 范围地址会匹配注释里的同字样;删 TOML 段落别用模糊 sed,要么精确锚定、要么本地改好整份传。
- **状态**:env 就绪(连接 / venv / torch-cpu / 检索库 / PG17)。**待续**:建 PG 库、装 Qdrant(aarch64 binary)、ingest、**CPU spike(阶段0命门)**。
- **关联**:`~/.ssh/config`(pi 别名)· Windows portproxy · `/tmp/pyproject_pi.toml`(Pi CPU 变体)· memory `project-pi-bringup`/`project-exhibit-deployment` · plan `enchanted-questing-shore.md`

### P-023 · Phase 5 · Pi 展台阶段0:CPU 检索命门实测 + reranker 提速可配 + RAG device 收口  ✅
- **背景**:承接 P-022 的 env 就绪,跑完阶段0——基础设施落地 + CPU 检索可行性(单机方案命门)+ 提速 + 端到端闭环验证。
- **基础设施**:PG17 原生建 `ieqops` 库 + 8 表、Qdrant **v1.18.2 aarch64 binary** + systemd(开机自启/崩溃自拉)、`ingest --source corpus` **824 chunks**(CPU fp32,~75min,status green)。
- **CPU spike**(新 `ops/scripts/pi_retrieve_spike.py`,仿 vram_spike,stdlib 量 RSS+时延):**① 内存过关**——完整 retrieve 进程峰值 **3.8-4.0 GiB 零 swap**,8GB 足(权重 safetensors mmap,加载峰值才 1.1GiB);**② 速度是真问题**——单次 retrieve **~189s**,分段定位 **99% 在 reranker**(embed 1.4s / dense 0.08s / bm25 0.01s / **rerank30=187s**,每候选 ~6s 完全线性)。
- **提速杠杆逐个验真,只一个有效**:① **砍候选池 = 唯一配置级杠杆**(线性);② **max_length 512→256 无效**——chunk 仅切 500 字符 ≈125 token,代码 `padding=True` 动态 pad 到实际长度,512 从没触发截断,砍了不提速(以为能提速实际不能);③ 多线程**已吃满**(load 3.97 / 4 核)。
- **落地**:候选池可配 `RAG_RERANK_CANDIDATES`(`core/config.py`→`server.py`→`RetrievalStack.candidate_top_k`,**默认 30 护评测基线 / Pi `.env`=5**)。**实测 candidate=5:189s→24.4s(7.7x)**,内存峰值还降到 3.5GiB。质量抽查四域 top1 全命中真值(thermal 精确 21-25°C、lighting CIBSE SLL、acoustic masking),**唯一苗头** airquality top1 偏到通风条款而非最核心 CO2 阈值(candidate=5==FINAL_TOP_K,reranker 退化为"给 5 个排序"、不再从大池精选——thermal top1 logit 负值印证)。
- **坑 · `IEQ_RAG_DEVICE` 不读 `.env`**:`mcp-rag-server` 原走 `os.getenv("IEQ_RAG_DEVICE","cuda")`(**OS 环境变量,不是 pydantic `.env`**);Pi 的 `.env` 写 cpu 但走 server 的生产路径读不到 → 无 GPU 的 Pi 会因 `device="cuda"` 在 `.to()` 当场崩。spike 没踩到是它直接传 `device="cpu"` 绕过 server。**收口**:加 `settings.rag_device`(`AliasChoices` 同认新 `RAG_DEVICE` + 旧 `IEQ_RAG_DEVICE`,向后兼容现有 `.env`/memory),server.py 改读 settings、去掉 `import os`。systemd 服务化不再需要 shell export。
- **验证**:Pi 端到端 `demo.py co2_spike` **两次关单**(closed/met,co2 1300→679.4,delta −220.6),记忆轨迹也写入 `ieq_incidents`。**收口后不传任何 shell env、纯靠 `.env` 经 alias 读 cpu** 仍 cpu 关单(`I-20260614-R1-AQ-011130`),证明 systemd 不 export 也走 cpu。LangFuse 上传失败(Pi 没起该服务)**fail-open 不阻断闭环**,展台本不需要。
- **教训**:① 切部署平台是一条「env 读取路径」回归链——`os.getenv` vs pydantic Settings 的不一致只在跨机时暴露,配置项要统一收口进 Settings;② 提速前先量分段,别拍脑袋(max_length 看着该砍其实无效,真瓶颈是候选数);③ 牺牲准确率换速度要标清退化边界(candidate==FINAL_TOP_K 时 reranker 名存实亡)。
- **关联**:`ops/scripts/pi_retrieve_spike.py` · `rag/retrieve.py`(candidate_top_k)· `core/config.py`(rag_device/rag_rerank_candidates)· `mcp_servers/rag/server.py` · memory `project_pi_cpu_retrieval` · plan `enchanted-questing-shore.md` 阶段0

### P-024 · Phase 3 · 闭环补完:critic 否决 / verifier missed 接 replan(不再 END/FAILED 静默)  ✅
- **背景**:CLAUDE.md 原则 #2「失败的干预触发 replan,而非沉默」一直只兑现一半——`_route_after_critic` 的 Tier1/2 否决直接 END、verifier missed 直接 FAILED,`replan_count` 字段恒 0 从没递增。本次把两条死胡同接回 planner,补完闭环。展台无依赖硬件,纯后端 + demo 可验。
- **难点不在路由,在「重试要清空上一轮」**:`subtask_results`(merge reducer)和 `failed_subtasks`(append reducer)是为 specialist 并行 fan-out 设计的累加通道;直接 `return {}` / `[]` 会被 reducer 合并成 no-op,旧结果残留 → 新 plan 的 `S1` 被当成「已解决」永不重派,primary_result 还会取到上一轮的陈旧诊断。
- **解法 · reset 哨兵**:`_ResetResults(dict)` / `_ResetList(list)` 子类哨兵——reducer 收到它就**替换而非累加**(`isinstance` 判定);因其本身就是 dict/list,LangGraph 的 Pydantic state 通道校验仍通过(reducer 把它归一化回普通容器)。**单测证实**:经真实 `StateGraph(MainIncidentState)` 编译运行,节点 return 哨兵后通道确被清空、无校验报错(这是动手前最担心的点)。
- **预算与一致性**:`MAX_REPLANS=2`(共 3 次 planner 运行)+ `replans_left(replan_count)`。critic 和 verifier 用**同一个** `replans_left` 决定 retry-vs-fail,`replan` 节点是**唯一**递增 `replan_count` 的地方 → 它俩看到的预算与下游路由一致。critic 否决留预算时**不写 ticket FAILED**(在重试,不是失败);verifier missed 留预算时 status=PLANNING 且**不写 episodic 轨迹**(中途的 miss 不是事件结局,只有终态 met/exhausted-FAILED 才落 episodic)。
- **长跑链路同步**:`scheduler.resume_tick` 原本 resume 后无条件 `discard+delete_thread`——但 replan 会让同一 thread 在 verifier 前**二次挂起**,故改为 resume 后复查 `snap.next`,仍挂起就用新窗口 `register`(已是幂等 upsert)续命、不清理;`demo.py` 把单次 advance/resume 改成 **while 循环**(每次尝试推进一次模拟),`_accumulate` 改调真 reducer 与重置同步。
- **验证 · `co2_overcrowded` 新场景**(20人/50m³、通风已拉满 450m³/h、稳态≈1220ppm,单一执行器无解):一次跑**两条 replan 路径全覆盖**——① critic 否决(判 15min 达标不现实)→ replan attempt1;② replan 后 critic 批准 → 执行 → **verifier missed**(delta=320,current 恒 1220)→ replan attempt2;③ 再 missed → `replan_count=2` 预算耗尽 → **status=failed verdict=missed**。每次尝试 specialist 都重新跑(证明通道确被重置)、无死循环、终态唯一一条 episodic。`mypy --strict core/` 通过。
- **诚实边界**:这是「重试到耗尽后如实判 FAILED」的安全叙事,**不是**「重试后成功关单」——房间物理上高通风一个 15min 窗口就逼近稳态,靠多等窗口翻盘需要极高初始 CO2 且受 specialist 的 LLM 目标值影响、不易确定性复现;故选了「执行器压根够不着」的确定性失败路径来验证闭环。成功关单仍由既有 `co2_spike`(单窗达标、无 replan)承载。
- **关联**:`core/state.py`(reset 哨兵 + MAX_REPLANS + replans_left + reset_attempt_channels)· `core/graph.py`(replan 节点 + `_route_after_critic`/`_route_after_verifier`)· `agents/critic.py` · `agents/verifier.py` · `ops/scheduler.py` · `ops/scripts/demo.py` · `sensing/simulator/scenarios.py`(co2_overcrowded)

### P-025 · Phase 5 · 展台纯软件三连:运维面板 + scheduler systemd + 云 API 韧性  ✅
- **背景**:承接展台「A 类纯软件」清单(见 memory `project-exhibit-remaining-abc`),一口气清掉三个不依赖麦/喇叭/屏的项。
- **① 运维面板**(`frontend/api/ops.py` APIRouter 挂到既有问答网关 + `frontend/app/ops.html`,无 LLM):incident 实时流(轮询 3s,signature 变了才重渲染防闪)、按状态计数头、传感器条、点击 drill-down modal、**异常注入面板**(`POST /api/inject` arm 场景 + 后台单飞线程跑 `scan_tick`,让闭环现场按需触发,不必等真异常或 5min tick)。**坑 · 404 变 500**:`get_incident` 的 `ValueError` 经 in-memory MCP client 被重包成 `fastmcp.exceptions.ToolError`,我原 `except ValueError` 抓不到 → 改抓 `ToolError` 返 404。**坑 · nohup 被收割**:`nohup uvicorn & ` 写在 harness 后台命令里,wrapper 脚本一退就把 nohup 子进程带走了(exit 144);改用 harness 原生 `run_in_background` 跑 uvicorn 才常驻。**实测**:dev 跑 `co2_spike` 注入 → 完整 monitor→…→action→suspend 一条龙,新 incident 入流 status=verifying。
- **② scheduler systemd**(`ops/deployment/` 三个 unit):`ieq-scheduler.service`(常驻 scan+resume 心跳,Type=simple/Restart=on-failure,After/Wants pg+qdrant 尽力排序)+ `ieq-reflection.service`(oneshot)+ `ieq-reflection.timer`(`OnCalendar=Sun 03:00`、`Persistent=true` 补跑漏掉的周——Pi 夜里可能断电)。补上 run_reflection 一直留的「cron wiring is Phase 5」。运维面板**无需**新 web unit(只是 `frontend.api.main` 上加路由,既有 `ieq-web` 已服务)。`systemd-analyze verify` 过、uv 路径与既有 unit 一致。
- **③ 云 API 韧性**(`core/router.py` + `core/config.py`):命门是 **OpenAI client 默认 timeout=600s**——无人值守时一个卡住的连接能把 5min scan 堵十分钟。加 `LLM_TIMEOUT_S`(默认 120s)+ `LLM_MAX_RETRIES`(默认 2,SDK 对 conn/429/5xx 指数退避重试,先于 router 的换模型兜底)。另把 **429 改判为可兜底**(原作 `<500` 的客户端错直接 raise,但限流是瞬时的、换另一模型是另一容量池)——抽 `_status_is_retryable`(≥500 或 ==429)让 `complete`/`stream` 同步。`mypy --strict core/` 过,实测 client 取到 timeout=120/retries=2、happy path 不破。
- **关联**:`frontend/api/ops.py` · `frontend/app/ops.html` · `frontend/api/main.py` · `ops/deployment/ieq-{scheduler,reflection}.service` + `ieq-reflection.timer` · `core/router.py`(`_status_is_retryable`)· `core/config.py`(`llm_timeout_s`/`llm_max_retries`)· memory `project-exhibit-remaining-abc`。**剩**:Tier3 审批(需 graph-resume + 无 actuator 域语义决策)、72h dry-run(待全栈稳定)、语音/kiosk(半依赖硬件)。

### P-026 · Phase 5 · Tier3 审批上面板(**作者拍板:登记式,非 graph resume**)  ✅
- **真实架构岔口**:Tier3 的三域(thermal/lighting/acoustic)**没有 actuator**(Phase 5 才有),`action` 节点只会发通风指令(airquality 专用)。所以人"批准"后**没有真东西可执行**——若真 resume graph,approved 的 thermal 单会去跑 `set_ventilation`(错的执行器)。三选项摆给作者:① 人工已处置→关单(真 resume,需给 autonomy_gate 加「approved 无 actuator → 关单」路由)② 暂不做 ③ 仅面板登记关单(最快,但不真驱动 interrupt)。**作者选 ③**(明知"不推荐/不诚实"标注)。
- **登记式必须连贯,否则半坏**:Tier3 单在 `autonomy_gate` 的 `interrupt()` 处挂起、被 `scan_tick` 登记进 `suspended_threads`;若只 patch ticket 不清挂起线程,scheduler 的 `resume_tick` 15min 后会用 `None` 恢复它判 FAILED,**把人工 closed 覆盖**。故审批端点必须**同时丢弃挂起线程 + 删 checkpoint**。
- **落地**:① `autonomy_gate` 进 interrupt 前置 ticket=AWAITING_APPROVAL(否则面板只看到 critic 留的 ACTING,认不出待审)② `core/suspend.py` 加 `thread_for_incident(incident_id)` ③ `POST /api/incidents/{id}/decision`(approve|reject):patch ticket(closed+「resolved by human approval」/ failed)+ `thread_for_incident`→删 checkpoint(`open_checkpointer().delete_thread`)+ `discard` ④ `ops.html` detail modal 在 awaiting_approval 时显示批准/拒绝按钮,非待审单 409 守卫。
- **诚实标注**:代码 + commit 明写「bookkeeping, not a graph resume — 不驱动 interrupt(),durable interrupt 仍是架构的 Tier3 阻塞」。这是作者知情选择的、清晰标注的范围限制,非隐藏的假实现。真 resume 留到有 per-domain actuator。
- **验证**:注入 `overheating` → thermal 单停在 `awaiting_approval`(`autonomy_gate tier=3` → `incident_updated awaiting_approval` → `scan_suspended`);approve → `closed` +「resolved by human approval (Tier 3)」、挂起线程 `thread_for_incident`→None(scheduler 不会再 auto-fail)、再审 409、stats 待审归 0。`mypy --strict core/` + ruff 过。
- **关联**:`core/graph.py`(autonomy_gate)· `core/suspend.py`(thread_for_incident)· `frontend/api/ops.py`(decision 端点)· `frontend/app/ops.html` · 关联 [[feedback_impl_vs_plan]] 的诚实标注原则 · commit `0913e48`

### P-027 · Phase 5 · 展示路径隔离 replan(co2_overcrowded 移出面板,replan 留论文/CLI)  ✅
- **动机**:连 Pi 把功能完整跑了一遍(7 项全绿:基础设施 / 运维面板 / 问答 / RAG 检索 / 语音 TTS / scheduler / 闭环),其中 `co2_spike` 在 Pi 纯 CPU 上端到端**一遍过关单**(monitor→记忆召回→ReWOO DAG→Agentic RAG→critic→Tier1 执行→verify met→轨迹回写,~7.5min)。作者据此拍板:**现场展示只走一遍过,绝不出现 replan 过程**(replan 闭环留到论文再讲),保证准确、流畅、不出意外。
- **要隔离的就是 `co2_overcrowded`**:它是确定性 FAILED 场景(20 人 @50m³ + 通风拉满 450m³/h,CO2 稳态 ≈1220>900,单执行器无解)→ verifier 每窗都 missed → replan×2 → 耗尽预算判 FAILED。这正是展示最怕的"意外",必须从现场路径拿掉。
- **落地**(最小面):① `sensing/simulator/scenarios.py` 的 `Scenario` 加 `exhibit_safe: bool = True`,仅 `co2_overcrowded=False` ② `frontend/api/ops.py`:`/api/scenarios` 列表 `if s.exhibit_safe` 过滤、`/api/inject` 对非 exhibit-safe 场景返 **403**(防直接 POST 绕过面板)③ `demo.py` CLI **不变**——`--list` 仍列、`demo co2_overcrowded` 仍能跑,留给论文/作者复现 replan。
- **验证**(Pi 实跑):面板 `/api/scenarios` 只剩 `['co2_spike','overheating','dim_workspace','noisy_room']`、`co2_overcrowded hidden:True`;`POST /api/inject co2_overcrowded` → `403 "paper/CLI-only (not exhibit-safe)"`。ruff 过、`exhibit_safe` map 正确。
- **关联**:`sensing/simulator/scenarios.py`(exhibit_safe)· `frontend/api/ops.py`(/api/scenarios 过滤 + /api/inject 403)· demo.py CLI 保留 · memory `project-exhibit-remaining-abc`。**下一步**:排查 grade 总判"不充分"跑满 3 轮 rewrite 的主因(WELL chunk 质量 / grade 阈值偏严)+ 加速闭环(7.5min→更短),让现场更流畅。

### P-028 · Phase 5 · 展示加速:grade v2(职责归位)+ MAX_REWRITES=1(最坏时长上界)  ✅
- **背景**:连 Pi 实测闭环单 incident ~7.5min,现场偏慢。拆解发现两个 specialist 的 grade **每轮判"不充分"跑满 3 轮 rewrite** 是主因(各做 4 轮 RAG)。
- **根因(诊断 grade 真实 reason)**:grade v1 要求 excerpts **fully** answer the goal——含"补救数值 / 验证步骤 / 具名标准(ASHRAE 62.1)"。但标准文档(WELL)只给阈值/要求,**补救+验证是 generate 节点推理产出的**,不是检索目标;且 corpus 是 WELL 单源,grade 却期待 ASHRAE → 永远判缺。**grade 比 generate 严**,职责错配。
- **修法 ① grade v1→v2**(`ops/prompts/specialist/grade/v2.md` + builder 调用点 bump 到 2):判定对象从"覆盖整个 goal 含补救"收窄到"**是否有足够标准证据 ground 诊断**",明确不要求 excerpts 自带补救/验证/具名标准。保留"off-topic→false"质量门(防假阳性,守硬约束 #11)。**实证(开发机)**:同一批 13 chunk,v1 false(要补救数值)→ v2 true;再喂 off-topic(噪声/照明)chunk → v2 仍 false(没退化成无脑 true)。
- **修法 ② MAX_REWRITES 3→1**(`builder.py`):grade v2 后首轮 sufficient 率高,真正还触发 rewrite 的是 **corpus 薄的 domain**(thermal/lighting/acoustic thresholds 待对齐 P-016)——而 rewrite 救不了 corpus 缺的证据(每轮都 false→最终 generate fallback)。降到 1 给闭环时长一个上界,不损结果质量。CLAUDE.md "capped at 3" 是上限,1≤3 不违反。
- **Pi 实测**:co2_spike `7.5min → 2:15`(S1 grade round-0 sufficient、met 关单质量无损)。其中 ~104s 是两次模型冷加载(memory bge-m3 31s + reranker 73s)——走**常驻 gateway / 面板注入**时 lazy-singleton 只加载一次,**展示前预热一次**后续 incident ~40s(不必改代码,运维技巧)。
- **遗留**:① planner 非确定(co2_spike 有时单 S1、有时 S1+S2,本次因 episodic 召回 3 个成功旧案简化为 S1——记忆生效的副产物)② thermal/lighting/acoustic corpus 薄(根治要补 corpus,P-016)。两者都由 MAX_REWRITES=1 兜底(最坏 ~3min)。
- **关联**:`ops/prompts/specialist/grade/v2.md` · `agents/specialists/builder.py`(grade v2 调用点 + MAX_REWRITES) · memory `project-exhibit-remaining-abc`、`project-corpus-source`。

### P-029 · Phase 5 · 展台成品收尾:scheduler 上 Pi 常驻 + 命门(resume 推进模拟器) + 跨进程锁 + 合并 kiosk + Tier1/Tier3 双闭环 Pi 实测  ✅
- **背景**:把展台做成「能自主跑 + 能造异常 + 能点按钮对话」的成品。三需求落地:① scheduler 每 5min 自主扫描、有异常发 incident ② 面板注入异常 ③ kiosk 点按钮语音问答。摸清现状后真正缺口比想象小——scheduler 代码早有,但**没上 Pi 且 unit 是开发机路径**(`/home/xms` + `uv run` 会触发重新 sync);注入面板/语音 UI 已具备。外设按作者定:先用开发机屏/麦/喇叭(浏览器 `localhost:8000`,核心全在 Pi),买的到了直接接 Pi 不改代码。
- **命门(否则 replan 翻车)**:模拟器只认 `advance_minutes()` 不认墙钟,而 `scheduler.resume_tick` 原注释明写「does NOT advance the simulator」(原设计假设跑在有真传感器的机器)。展台用模拟器时,注入 co2=1300→开通风→15min 后 verifier 去读,模拟器里**还是 1300**(没人推进它演化)→判 missed→触发 replan,正是现场最怕的意外。**修法**:`sensor_source=="sim"` 时 resume 在 invoke verifier 前 `reload_room()` + `advance_minutes(target)` 把房间快进过验证窗口(CO2 真降到 ~680);`hardware` 模式不碰(真传感器自己变,对齐 C 类 sim/hardware 切换)。**只对挂在 `verifier` 的 thread 推进**——Tier3 挂在 `autonomy_gate`(等人,非等时间)绝不推进(`if sim and "verifier" in snap.next`)。
- **压缩窗口**:`exhibit_mode=True` 把墙钟 resume 等待从真实 15min 压到 `verify_window_seconds=90`,观众站着就能看完关单(模拟器瞬间快进 15min,墙钟只留 ~90s「系统在观察效果」的停顿)。三开关默认即展台配置,Pi 无需改 `.env`;论文/真部署再翻 False。
- **跨进程锁(否则 OOM)**:scheduler 进程的 5min 心跳 scan 与 web 进程的注入 scan 都跑 `scan_tick`,各拉一份 RAG(~3.8GiB)→ 8GB Pi 撑爆。原 `_scan_lock`(threading)只在 web 进程内有效。**加 Postgres SESSION advisory lock**(全机单飞):web 注入 `wait=True` 阻塞等锁(绝不丢注入;无异常心跳 scan 在 Monitor 判完即释放,秒级),scheduler 心跳 `try` 锁撞上就跳(dedup 兜底)。锁持在 pooled 连接上、显式 `pg_advisory_unlock`(pool reset 的 rollback 不释放 session 级锁)。
- **跨进程模拟器一致性**:web 进程 arm+action 写状态文件(`save_room`),scheduler 进程 resume 读——原 `get_room()` 单例缓存会读到 stale。加 `reload_room()`(置空缓存重读文件,文件 `IEQ_SIM_STATE` 是跨进程单一真相),scan/resume 推进前都先 reload。
- **Tier3 不用新写**:`_TIER_BY_DOMAIN` 只有 airquality=Tier1,thermal/lighting/acoustic 默认 Tier3(`tier_for_sensor`→APPROVE)——现成的 `overheating`(29°C)注入就挂 `awaiting_approval`。给 `/api/scenarios` 加 `tier` 字段(从 `tier_for_sensor` 推导,**不引第二真相源**)让面板区分 AUTO / NEEDS APPROVAL。
- **合并 kiosk 大屏**(`/kiosk` + `frontend/app/kiosk.html`):融 `ops.html`(incident 实时流+注入+审批卡)与 `index.html`(级联语音问答),一屏让观众同时看「系统自主处理异常」+「开口问它」。注入按钮带 tier 徽章;Tier3 挂起时右侧浮出橙色审批卡(轮询 `awaiting_approval`,点批准走既有 `/decision` 登记式端点);语音 push-to-talk(开发机麦/喇叭、Pi 跑 STT→LLM→TTS)。保留原 `/`、`/ops` 两页不破坏。
- **Pi 部署**:`ieq-scheduler.service` Pi 版(`User=xms12138` / `~/dissertation` / `.venv/bin/python -m ops.scheduler` / `IEQ_RAG_DEVICE=cpu`,展台开关走默认)→ install+enable;rsync 排除 `pyproject.toml`/`uv.lock`;重启 `ieq-web` 加载新页。两服务 active 共存。`scheduler_start` 确认 `sensor_source=sim, exhibit_mode=true`。
- **Pi 端到端双闭环实测**:① **Tier1 `co2_spike`**:`closed/met/Δ−220.6`,scheduler 日志 `sim_advanced_for_verify co2=679.4`→`incident_updated closed`→`episodic_saved`→`resume_done`,完整闭环(注入→ReWOO→Agentic RAG→critic→Tier1 执行→推进模拟器→verifier met→关单→轨迹回写)。② **Tier3 `overheating`**:140s 挂 `awaiting_approval`(thermal 诊断+critic+gate Tier3),approve→`closed`「resolved by human approval」,**该 thread 的 scheduler 日志为空**——证明人批在 90s 窗口前、thread 被 `/decision` 端点干净清除,`resume_tick` 全程没碰它(不推进、不误判)。
- **遗留 · 首次冷启动慢**:首注入 incident 在 `open` 停 ~240s(RAG bge-m3+reranker 首次加载 ~104s,P-028 已记),预热后 ~40-140s。**展前预热**:开展前先注入一次 `co2_spike` 让 web 进程的 RAG singleton 加载,后续 incident 快。scheduler 进程是另一份 singleton,但注入恒走 web 进程,自主 scan 遇真异常才加载(罕见)。
- **关联**:`core/config.py`(exhibit_mode/verify_window_seconds/sensor_source)· `sensing/simulator/room.py`(reload_room)· `ops/scheduler.py`(advisory lock + _resume_delta + _advance_sim_before_verify + docstring 更新)· `frontend/api/ops.py`(scan_tick wait=True + /api/scenarios tier + /kiosk 路由)· `frontend/app/kiosk.html` · `ops/deployment` Pi scheduler unit · memory `project-exhibit-remaining-abc`、`project-pi-cpu-retrieval`。

### P-030 · Phase 5 · 展示透明化:自主巡检可见 + 处理详情(plan/RAG证据/诊断)上屏 + 界面精简  ✅
- **背景**:把展台从「能跑」推到「看得懂」——让观众看清 agent 如何工作。三件:① 自主巡检可见(无异常时也证明系统在巡检)② incidents 精简(3 条去细节)③ 新增「How it works」面板显示当前 incident 的 plan→RAG 检索证据→诊断。前置:已加打断语音的 ⏹ 按钮(AbortController 掐流 + PcmPlayer.stop/speechSynthesis.cancel,`60d39c1`);界面英文化 + 传感器 gauge + pipeline 进度(`d640090`)。
- **命门 · 子图隔离 vs 展示需求**:RAG 检索到的 `retrieved_chunks` 在 specialist 子图里**故意隔离**(CLAUDE.md:不进父 checkpoint,防 state 膨胀 + Plan B 核心),父图本看不到,没法直接上屏。**解 = 展示旁路**:新表 `incident_steps`(plan/evidence/diagnosis 分步),`generate` 节点拿到 chunks 时**直接旁路写 DB**(不回流父 state)。incident_id 经 `dispatch` 的 `Send` payload → `run_specialist` → 子图 state 一个**标量** `incident_id`(非 bulky,不违反防膨胀初衷)传进去。既显示真实检索内容,又**完全不破坏**「子图状态不污染父图 checkpoint」的设计。
- **写入点**:① plan——`route_dispatch` 第一波(`subtask_results` 还空、replan 会 reset 故每 plan 一次)写 ReWOO DAG ② evidence+diagnosis——子图 `generate` 写检索 chunks 摘要 + 诊断。全 best-effort try/except 包裹,**展示写失败绝不断闭环**(observability 不是控制流)。`record_step`/`list_steps` 是 ticket server 普通函数(非 @mcp.tool,内部调用)。
- **自主巡检可见**:`scan_tick` 记 `scheduler_heartbeat`(单行 upsert,`if not wait` 只记自主巡检、手动注入不算);`/api/scheduler/status` 给 last_scan + interval;kiosk header `last scan Xs ago · next in Ym Zs` 倒计时 + 扫描脉冲。**首次 scan `next_run_time=now` 立即跑**(启动即有 heartbeat + 预热 RAG),配合部署时重置模拟器避免首扫撞残留异常。
- **kiosk 重构**:incidents 5→3 条 + 卡片去 id/action 文本(只留 sensor/value/status/pipeline、verdict 精简);新增 How it works 面板(左列下半 flex,跟踪最新 incident 的 steps,plan/evidence/diagnosis 三块随 pipeline 逐步出现);band 颜色来自新 `/api/thresholds`(跟 Monitor 同源)。
- **Pi 实测(真实写入,非编造)**:注入 co2_spike → **plan ~60s 出现**(本次 planner 生成 S1+S2,含 thermal 副作用评估 + ReWOO `#S1.diagnosis` 跨域依赖)→ **evidence+diagnosis ~160s**。evidence 是真实 WELL v2 chunks(Increase Outdoor Air Supply、900ppm 阈值表、demand-controlled ventilation),diagnosis grounded 到 excerpt 3,4。证明展示旁路拿到的是真检索内容。
- **诚实边界**:evidence 显示**真实**累积检索 chunks(多轮/多 sub-query 累积,含少量参考文献噪声 chunk)——这是 RAG 真实行为,不美化;前端每 chunk 截 280 字 + 滚动。`incident_steps` 是展示旁路,与论文的 LangFuse trace 是两套(展台 Pi 没起 LangFuse)。
- **关联**:`mcp_servers/ticket/server.py`(incident_steps/scheduler_heartbeat + record_step/list_steps/record_scan/get_last_scan)· `core/graph.py`(route_dispatch 写 plan + Send incident_id + _record_plan)· `agents/specialists/builder.py`(SpecialistState.incident_id + generate 写 evidence/diagnosis + _record_evidence)· `ops/scheduler.py`(record_scan + next_run_time)· `frontend/api/ops.py`(scheduler/status + incident/steps 端点)· `frontend/app/kiosk.html`(patrol + How it works + 3 条精简)· memory `project-exhibit-remaining-abc`

### P-031 · Phase 5 · 断电/掉电韧性加固:模拟器状态挪出 /tmp + systemd Wants 拉起 DB  ✅
- **背景**:作者问「Pi 断电 / 传感器断电后整套还能正常工作吗」。审查后分两场景诚实作答 + 修两个真实脆弱点(纯本地改 + 本地验证,Pi 关机期间完成,待开机 rsync 复验)。
- **场景梳理**:① Pi 整机断电——三服务 `enabled` 开机自启 + `Restart=on-failure` 自愈 + PG/Qdrant 磁盘持久 + 挂起线程在 `suspended_threads`(PG)→ 闭环能跨重启续跑(checkpointer 设计)。② 传感器断电——当前 sim 模式 `read_sensors` 纯读模拟器、**无 hardware 分支/无 MQTT 掉线容错**(注释写 Phase 5 才 swap),接真 Arduino 才需补 stale 检测 + `sensor_offline` 工单。
- **脆弱点① · sim 状态在 /tmp 断电丢**:`room.py` 默认 `IEQ_SIM_STATE=/tmp/...`,只扛得住进程 kill(同次开机 /tmp 还在),**扛不住 reboot**(tmpfs/开机清)。后果:断电打断进行中闭环时,`resume_tick`→`reload_room()`(`scheduler.py:117`)读不到 web 进程持久的 post-action 房间(开窗后 ventilation),Verifier 读到错误物理态。**修**:默认路径 → repo `var/sim_state.json`(Pi 上 `/home/xms12138/dissertation/var/`,持久磁盘),`save_room` 首写 `mkdir(parents)`,`IEQ_SIM_STATE` 仍可覆盖(测试指 tmp_path)。`.gitignore` 加 `var/`。本地 PASS(默认路径解析 + save 建目录 + reload 读回一致)。
- **脆弱点② · postgresql 自启未坐实**:`ieq-web` 单元**完全没声明** DB 依赖、`ieq-scheduler` 只 `After=`(排序)不 `Wants=`(拉起)。若 `postgresql.service` 没单独 enable,开机 DB 不起 → web/scheduler 连不上。**修**:两单元加 `Wants=postgresql.service qdrant.service`——纵深防御:**enable web/scheduler 时 systemd 会在 boot 顺带拉起 DB,即使 DB 没单独 enable**;注释补 `systemctl enable postgresql qdrant` 双保险。
- **审查 · scheduler.py 通读**:核心健壮无真 bug;本次加固恰好补上断电恢复链最后一环(`reload_room` 现能跨断电读到 post-action 房间)。1 个**可选优化**(未改,待确认+Pi 验证):`resume_tick` job 无 `next_run_time=now`(`scheduler.py:213`),断电重启后挂起 incident 最多等 1min 才被捡起(scan 有立即首跑、resume 没有)。
- **诚实边界**:② 是**模板改动**(repo 单元是开发机版 `/home/xms`+`uv run`,Pi 跑特化版 `xms12138`+`.venv`),**Pi 开机后须把改动同步到 Pi 的 `/etc/systemd/system/` 那份 + `daemon-reload` + 重 enable 才生效**;`postgresql` 是否已 enabled 仍需 Pi 开机 `systemctl is-enabled postgresql` 坐实(本次因 ssh 隧道断未确认);传感器离线容错接真硬件再做。
- **关联**:`sensing/simulator/room.py`(_DEFAULT_STATE_PATH + save mkdir + docstring)· `.gitignore`(var/)· `ops/deployment/ieq-web.service`+`ieq-scheduler.service`(Wants DB + enable 注释)· `ops/scheduler.py`(reload_room 断电恢复链)· memory `project-exhibit-remaining-abc`

### P-032 · Phase 5 · Pi 自启 kiosk(cage + chromium 全屏)——头号成品缺口①落地,只差一块屏  ✅
- **背景**:把 Pi 从「一套在跑的服务」变成「开机即用 appliance」——作者要「显示屏插上、Pi 开机就进工作界面,不再依赖开发机/隧道」。这是 [[project-exhibit-remaining-abc]] 的 A 档头号缺口①。Pi 是 Debian13 trixie **Lite**(实为装了 **RPi 官方源**,包带 `+rpt`):无桌面/无浏览器/无 kiosk 自启,kiosk 此前只能从开发机经 ssh 隧道看。
- **方案 = cage + chromium**(都来自 RPi 源,硬件优化版):`cage` 是极简 **Wayland kiosk 合成器**(单一全屏应用、为 kiosk 而生,比 labwc/wayfire 桌面轻得多);chromium RPi 优化版。`/dev/dri/card0/card1` DRM 输出在,用户 `xms12138` **已在 `video render input` 组**(cage 访问显卡/输入正好需要)。
- **seat 权限链路(天然就通)**:`seatd` 提供 DRM/input seat;`/run/seatd.sock` **属组 `video`**,用户已在 video 组 → 无需额外加组。`loginctl enable-linger xms12138` 让 `/run/user/1000` 常驻(kiosk service 不依赖交互登录会话)。
- **麦克风免隧道**:chromium **managed policy**(`/etc/chromium/policies/managed/ieq-kiosk.json`)预授权 `AudioCaptureAllowedUrls=[http://localhost:8000]` → 无人值守 kiosk **不弹权限框**。浏览器跑在 Pi 本机、访问 `localhost` = **secure context**,麦克风**不需要任何隧道**(这正是浏览器要跑在 Pi 上、而非开发机的根本原因)。
- **service**:`ieq-kiosk.service` —— `TTYPath=/dev/tty1`+`PAMName=login`(cage 拿 VT/session)、`Conflicts=getty@tty1`(接管控制台)、`After/Wants=ieq-web+seatd`、`XDG_RUNTIME_DIR=/run/user/1000`、`Restart=on-failure`。`enabled`(开机自启)。
- **无显示器验证(关键)**:`systemctl start` 后 `active(running)`、`NRestarts=0`、**零报错**;`ps` 确认 `cage`(5912)+`chromium` 多进程全活;`/run/user/1000/wayland-0` 合成器 socket 就绪。即合成器+浏览器**全就绪**,cage **健康挂起等 DRM connector(热插拔)** → **插屏即亮,无需手动**。验证完 **stop 省资源 + 保持 enabled**(显示器还没到货,不空耗 chromium 内存)。
- **诚实边界**:① **「全屏渲染那一下」必须显示器插上才能肉眼验**(无 connector 无法验画面)——已做到「插上即亮」的最大程度。② 麦克风/喇叭真机验证同样等外设到货。③ policy 预授权写死 `localhost:8000`,若 web 端口/URL 变要同步改 policy。交接:显示器插上 → 开机(或 `sudo systemctl start ieq-kiosk`)→ 自动全屏 kiosk + 麦克风可用,**全程不需要开发机**。
- **关联**:`ops/deployment/kiosk-start.sh`(cage--chromium 启动)· `ops/deployment/ieq-kiosk.service`(tty1+seatd+linger)· `ops/deployment/chromium-kiosk-policy.json`(麦克风预授权)· seatd · memory [[project-exhibit-remaining-abc]] [[project-voice-cascade]] [[feedback-hardware-handoff]]

### P-033 · Phase 5 · 语音真机收口:Lite OS 缺音频栈 + emoji 字体,装 PipeWire 后 chromium 须重启  ✅
- **现象**:外设(USB 麦 + 3.5mm 喇叭 + 触摸屏)接上 Pi、kiosk 正常显示,但 ① 点 🎤 语音问答整条链路用不了 ② 界面 🎤 显示成一个空框(豆腐块)。
- **根因**:Debian 13 **Lite** 镜像 ① **没装任何用户级音频服务器**(`pipewire`/`pulseaudio`/`pactl` 全 MISSING,只有底层 ALSA)——chromium 在 Linux 上的 `getUserMedia`(麦克风)与音频播放**实际依赖 PulseAudio/PipeWire**,裸 ALSA 下枚举不到设备;② **没装彩色 emoji 字体**(`fc-list | grep emoji` = 0),🎤 这种 emoji 无字形可渲染 → 豆腐块。两者都是"无桌面环境"的副作用:连"桌面默认有"的音频栈和 emoji 字体都缺。
- **解决**:`apt install pipewire pipewire-pulse wireplumber pipewire-audio pulseaudio-utils fonts-noto-color-emoji`。PipeWire 作为 **user service** 跑(Linger 已开 P-032,与 `/run/user/1000` kiosk 会话同 runtime),wireplumber **自动选默认设备**(USB mic=source、3.5mm jack=sink);unmute+85% 后录放实测电平 **RMS 0.21 / peak ~1.0**(真语音信号,非底噪)。**命门:chromium 在装 PipeWire 之前启动 → 探测不到音频后端,必须 `systemctl restart ieq-kiosk`** 才接上。重启后真机端到端跑通:push-to-talk → DashScope STT → DeepSeek → DashScope TTS → 扬声器朗读。固化为 `ops/deployment/setup-audio.sh`(幂等)。
- **教训**:① **Lite 镜像「能跑服务」≠「能跑桌面外设」**——接外设前要单独补音频服务器/字体这类桌面默认件。② **chromium 音频后端在启动期探测**:任何在 chromium 起来**之后**才装/改的音频栈都要重启浏览器才生效(区别于 P-030 的展示旁路——observability 改动不必重启,但音频是启动期能力)。③ 音频/字体是**系统层改动、不在 repo**,必须写成 setup 脚本 + DEVLOG,否则重装 Pi 全丢、不可复现(本条即固化)。④ 麦克风增益偏高(peak 触顶轻微削波),若日后 STT 准确率掉再降 source volume 到 ~65%。
- **关联**:`ops/deployment/setup-audio.sh`(新,幂等音频+字体)· `ops/deployment/chromium-kiosk-policy.json`(P-032 预授权麦克风,本条验证其在真机生效)· `ops/deployment/ieq-kiosk.service`(重启接音频)· `voice/provider.py`(DashScope 级联 P-021,本条是其 **Pi 真机首验**)· `EXECUTION_PLAN.md`「远程问答接入」阶段 A · memory [[project-voice-cascade]] [[project-exhibit-deployment]] [[feedback-hardware-handoff]]

### P-034 · Phase 5 · Arduino MKR1010 真传感器接入:固件烧录 + Pi Mosquitto broker，真数据达 broker  ✅(硬件→broker 通;ingest/read_sensors 切换待续)
- **背景**:承接接力 3→2→1 的「2 真数据」。作者接好硬件(MKR WiFi 1010 + SCD-30 I2C + Grove Light v1.1/A0 + Grove Sound v1.6/A1,均走 3.3V 免电平转换),板子挂在 Windows COM 口。
- **实际硬件偏离 2026-06-08 方案**:方案写 SCD40/BH1750/DHT22,实际用**已在手的 SCD-30(一颗覆盖 CO2+温+湿)+ 两个 Grove 模拟传感器**(光/声相对值,作者接受)。声音传感器一度考虑 LM393 数字版,**否掉**——数字版只输出阈值二值(拍手开关),给不出连续 `noise_db` 浮点,会废掉 WELL 分级阈值(50/55/60 dBA)+ 时间序列统计(avg/min/max 在二值上无意义),只有 Grove v1.6 的模拟值能要。
- **固件**(`sensing/hardware/mkr1010_node/mkr1010_node.ino`,WiFiNINA + ArduinoMqttClient + SparkFun SCD30):每 5s 发 JSON 到 `ieq/readings`,key 对齐 `SENSOR_COLUMNS`(co2/temperature/humidity)+ **`light_raw`/`sound_raw` 原始 0–1023**(标定刻意留 Pi 侧、免反复烧板);声音取 50ms 窗口峰值、光照 8 次平均;`arduino_secrets.h`(gitignore)装 WiFi/broker IP。
- **烧录踩的坑**:① **WSL 烧不了**——板子是 Windows COM 口,WSL2 默认无 USB 透传 → 工程拷到 Windows `D:\New\Dissertation\mrk1010` 由作者在 Arduino IDE 烧(我从 `/mnt/d` 同步 .ino + 校验内容一致)。② **库**:需 `ArduinoMqttClient` + `SparkFun SCD30`(作者原装的是 `Adafruit_SCD30`,API 不同,改装 SparkFun)。③ **`dtostrf` 编译报错**——AVR libc 的浮点转字符串函数,**SAMD(Cortex-M0+)核心不暴露** → 改用 `String(value, decimals)`(Arduino 核心内部自带、可移植)绕过,顺带省掉 char buffer。
- **Pi broker**:`apt install mosquitto`(2.0.21)。**坑:mosquitto 2.0 默认只监听 127.0.0.1 + 禁匿名** → 加 `/etc/mosquitto/conf.d/ieq.conf`(`listener 1883 0.0.0.0` + `allow_anonymous true`,LAN 内无鉴权、同列长期加固 #8 web auth);`systemctl enable` 自启。
- **真数据达 broker**:`mosquitto_sub -t ieq/readings` 收到 `{"co2":682.0,"temperature":35.80,"humidity":31.2,"light_raw":669,"sound_raw":249}`,且会跳变(SCD30 活的)。**利好:客户端隔离这次没挡 MQTT**——MKR(.153)↔Pi(.152)同网段直连成功,免上手机热点(`ssh pi` 当初要 Windows portproxy 是 WSL→Pi 那条路被隔离,WiFi 客户端互访这条路是通的,二者不同)。
- **数据待校**:`temperature` 35.8°C 明显偏高 = **SCD30 自发热偏移**(芯片贴 MKR 稳压电路,持续供电把读数抬高数度,是该传感器已知特性)——`setTemperatureOffset()`(固件)或 Pi 侧减偏移(更符合「标定留软件、不反复烧板」)二选一;light_raw/sound_raw 极值待采,以映射伪 lux/伪 dB 对齐 WELL 阈值(320 lux / 50 dBA)。
- **下次接力(硬件线收尾,纯 Pi 软件,作者不用碰板子)**:① `sensing/ingest/` 新写 MQTT 订阅 writer(paho 订 `ieq/readings`→标定→`record_reading`)② `read_sensors` 加 `SENSOR_SOURCE=sim|hardware` 开关(hardware 读 `sensor_readings` 最新行,sim 不变)③ 采标定极值 + 定温度偏移 → 真读数进表、问答管家/monitor 当场用上,multi-agent 上层零改。
- **关联**:`sensing/hardware/mkr1010_node/`(新固件)· `sensing/ingest/`(待写)· `mcp_servers/sensor/server.py::read_sensors`(待加开关)· `sensing/history.py::record_reading`(writer 复用)· `sensing/thresholds.py`(lux/noise_db 标定目标)· `/etc/mosquitto/conf.d/ieq.conf`(Pi,不在 repo)· EXECUTION_PLAN「硬件接入方案」+ checklist · memory [[project-hardware-deployment]] [[project-exhibit-remaining-abc]] [[feedback-hardware-handoff]]

### P-035 · Phase 5 · 真实数据全面接入(Hybrid:真实常态 + 注入演示)+ 开始长期跑  ✅
- **背景**:承接 P-034(真数据已达 broker),完成最后一段「broker → sensor_readings → read_sensors → 上层」的纯 Pi 软件,把展台从「sim 模拟数据」切到「Arduino 真实数据 7×24」,并保留手动注入异常做闭环演示。作者用选择器拍板 **Hybrid 数据模型**:真实数据是常态,注入是临时覆盖。
- **核心张力(切真实数据必然出现,先对齐再动手)**:① 注入原靠改模拟器,而真实模式 read_sensors 不读模拟器了 → 注入会失效;② **真实房间没有真执行器**(actuator 是模拟的),真 CO2 超标→「提高通风」假动作→真实 CO2 不降→verifier missed→replan→FAILED,长期跑会反复 FAILED;③ SCD30 自发热读 36°C → 不减偏移则切 hardware 第一次 scan 就误报 thermal。
- **Hybrid 解法(语义二分:真实世界 vs 注入覆盖世界)**:
  - **真实态**(`sensor_source==hardware` 且无 override,新 `sensing/override.is_live_world()`):monitor 检测真异常 → 诊断 → critic → **新 `advisory_close` 节点**:记录诊断 + 置 **新状态 `OBSERVING`**(非终态,dedup 拦住不重复建单),**不进 autonomy_gate/action/verifier、不 FAILED、不 Tier3 堆单**;环境恢复时 monitor `_reconcile_recovered` 自动关单(回 band→closed)。真实世界没有执行器可驱动,所以「诊断+观察」而非假装干预。
  - **注入覆盖态**:`/api/inject` 除 arm scenario 外,置 `sensing/override` 跨进程标记 → read_sensors 回落模拟器(注入+mock actuator+verifier 都在模拟世界操作)→ 走原 Tier1/Tier3 完整闭环 → 终态(scan/resume)`clear_if_resolved()` 清标记 → 回归真实数据。TTL 600s 兜底自愈(漏清不会卡在 sim)。
- **read_sensors 三态**(`mcp_servers/sensor/server.py`,契约名/输出形不变,上层零改):override→模拟器 / hardware→`sensing.history.latest_reading()`(读 sensor_readings 最新行,空表/NULL 回 in-band safe 默认)/ sim→模拟器。
- **ingest writer**(`sensing/ingest/`,paho 订 `ieq/readings`→`sensing.calibration.calibrate`→`record_reading`):温度减自发热偏移、light_raw/sound_raw 两点线性映射 lux/dBA;**未标定(极值 0/0)→返回 in-band safe 值**(420 lux/41 dBA),光声在标定前不参与异常判定,不乱报。**它是真实读数唯一写入者,取代 ops/sampler**(固件 5s 一帧比 sampler 密)。
- **部署坑**:① **Pi 服务用 `.venv/bin/python -m` 直接跑,不是 `uv run`**(查 Pi 现有 scheduler/web 的 ExecStart 才发现)——故 paho 用 `uv pip install` 进 venv,ingest unit 也用 venv python(避免 uv run 按 lock sync 卸掉 lock 外的 paho;Pi pyproject 是 cpu-aarch64 适配版,不 rsync,paho 不进其 lock)。② rsync 排除 pyproject/uv.lock/.env/var/缓存(.mypy_cache/.ruff_cache/.claude/*.wav)。③ **温度偏移 9.4**(SCD30 36.4 − 室温 27,作者给参照:室外 27、室内无空调无风扇);**27°C 本身超 WELL 上限 21-25**——真实热不适,系统报它是对的(但 co2 优先级在前,monitor 一次只报一个,co2 回 band 后才轮到 temp)。④ SCD30 温度偏高致 RH 读偏低(33-38%,Pi 侧减温度不修湿度),暂在 band 内不处理,记为已知限制。
- **验证(Pi 真机)**:`read_sensors` 走 hardware 返真实值(co2 781/temp 25.0/hum 38.3/lux 41 safe)、`live_world=True`;scheduler 切换后即时 scan→`monitor_no_anomaly`(当前 in band 不误报);注入 override 数据切换实测:inject→读 co2 **1300**(sim)+`live_world False`、clear→读 **781**(真实)+`live_world True`;graph 编译含 advisory 节点、标定/override smoke 全过;ruff+mypy(strict core)全绿。**长期跑已起**:ingest/scheduler/web 三个 systemd 服务 enable+Restart,开机自启。
- **光声标定(2026-06-27 现场采,作者配合制造极值我读+填)**:① ✅ 四极值已填 Pi `.env` 上线——`light_raw` 4(遮黑)/667(手电照)→ 0/600 lux、`sound_raw` 380(安静)/863(拍手)→ 40/75 dBA;实测 read_sensors `lux 400 / noise_db 40`(真实两点线性映射,均在 band 不误报)。判定逻辑:lux<320(raw≲358)才报 dim、noise>50(raw≳540)才报 noisy。**这组值是该传感器/该摆位特定的,重装 Pi 或挪动传感器需重采**(.env 不在 repo,gitignore)。
- **仍待续(不阻塞长跑)**:② 温度偏移精调(待温度计,现用 9.4);③ 可选湿度修正(SCD30 温高致 RH 偏低);④ 完整真实注入闭环现场点 kiosk 注入按钮验一次(override 闭环路径未改,P-033 已验过 co2_spike 完整关单)。
- **关联**:`sensing/ingest/`·`sensing/calibration.py`·`sensing/override.py`(新)· `mcp_servers/sensor/server.py::read_sensors`·`sensing/history.py::latest_reading`·`agents/monitor.py::_reconcile_recovered`·`core/graph.py::advisory_close`·`core/state.py::OBSERVING`·`frontend/api/ops.py`(inject 接 override)·`ops/scheduler.py`(终态 clear)·`ops/deployment/ieq-ingest.service`(新)· Pi `.env`(SENSOR_SOURCE=hardware/TEMP_OFFSET_C=9.4)· memory [[project-hardware-deployment]] [[project-exhibit-remaining-abc]]

### P-036 · Phase 5 · 真实数据上线后的调优、踩坑与韧性实测  🔧
承接 P-035,真实数据跑起来后一轮现场调优 + 暴露并处理几个真问题:
- **温度阈值放宽 + 偏移修正**:作者反馈欧洲夏天无空调室内 ~30°C、WELL 上限 25→真实环境持续超标。① 阈值 high 25→32(`thresholds.py`,rule 文本诚实标注「展台夏季放宽、标准仍 25」);② 温度偏移 9.4 是基于早先误估的「室温 27」算的,实际 ~30→真实 offset≈4(SCD30 raw 33.97−30;9.4 偏大把显示压低 5°C),改 **9.4→4**,显示回真实(随昼夜:凌晨 ~27、午后 ~30)。Pi `.env` TEMP_OFFSET_C 9.4→4。
- **前端阈值缓存 bug**(`kiosk.html`):改阈值后屏幕仍把 28.7 标红。根因 `loadThresholds()` 只在 page load 拉一次存 `THRESH`,定时刷新只刷读数→chromium 缓存旧 25。修:加 `setInterval(loadThresholds, 30000)`,阈值变更自动反映、免重启 kiosk。
- **transformers 回归(uv run 副作用)**:一句 `uv run python -c "import paho"` 触发 uv 按 lock sync,把 transformers 升回 5.10.2→`PreTrainedModel` 崩→specialist 诊断 + 注入闭环全废。降回 5.9.0(`uv pip install`)。**固化进 memory [[project-pi-bringup]]:Pi 别敲 uv run/sync,服务用 `.venv/bin/python` 正为绕开它**。
- **注入孤儿单(发现,未修)**:注入闭环若中途失败(单卡 open 未到终态),`clear_if_resolved` 因该 sensor 仍有 active 单而永不清 override→read_sensors 持续读 sim 假值直到 TTL 600s 兜底。实测撞上一次(co2 单卡 open 约 1h、override 盖住真实数据);已手动清,防御性兜底待补。
- **历史 backfill**(`ops/scripts/backfill_history.py`):合成两周历史(基线 + 日夜周期)+ 清掉切换早期 offset=0 的 3072 行脏温度,使「过去一周平均」等统计有真实感料。
- **韧性实测(作者主动拔插 MKR)**:作者拔掉 MKR 测鲁棒性——掉线 40min 期间 read_sensors 静默读 40min 前旧值(stale,无新鲜度检测,作者决定不补);插回后 **ingest 自动重连、数据流自愈**,零干预。✓
- **A–E 问答真实数据下实测全通**:当前读数 / 历史统计 / 异常历史 / 记忆建议(靠 incident 历史归纳,reflection 产的 fact/SOP 仍空)/ 越界拒答。
- 关联:`sensing/thresholds.py`·`frontend/app/kiosk.html`·`ops/scripts/backfill_history.py`·[[project-pi-bringup]]·[[project-exhibit-remaining-abc]]

**🆕 2026-06-27(续·真实数据下三个韧性 bug:verify 推进 gate + 注入 override 竞态 + CO2 坏帧/滞回):** 远程 SSH 接管 Pi 排查现场问题,从「注入 co2 第一遍 verify 没过」一路链式挖出三个**互相独立**的 bug,全部修复 + 实测:
- **① verify 不推进模拟器(hardware+注入态,头号 bug)**:作者注入 co2_spike 第一遍 verify 总 missed。根因——`scheduler.resume_tick` 的「verify 前快进模拟器过验证窗」步骤 gate 写的是 `sensor_source=="sim"`,而展台是 `hardware`;注入覆盖期世界其实是模拟器(read_sensors 回落 sim),但快进被跳过 → ventilation 动作生效了、模拟器时间没走 → verifier 读到**冻结的 1300**(delta 400)→ missed → replan。之所以最终还是关单,是 **override TTL 600s 到期、真实数据(恰在带内)接管**自愈,**不是动作起效**——met/missed 纯看「TTL 超时 vs Pi 慢闭环跑到 verify」的竞速(日志铁证:missed 时 `current=1300` 原值未动)。修:gate 改 `not is_live_world()`(纯 sim 展台 + hardware+注入覆盖都推进,真实无注入态不碰模拟器)。验证:受控单进程 `sim_advanced 1300→679.4 → verifier met`;活体路径再证 `1300→625.2 → met`、第一遍即关单无 replan。`ops/scheduler.py`。
- **② 注入 override 并发竞态(承接本条 P-036 上文「孤儿单」的另一面)**:活体注入偶发「点了没反应」——注入 scan 读到真实硬件而非 sim 1300。根因 TOCTOU:`/api/inject` 先 arm override **再**由其 scan 建单,这个窗口里 `active_incident_for_sensor` 还是 None,任何并发终态 scan 的 `clear_if_resolved()` 会把刚 armed 的 override 清掉 → 注入 demo 静默回落真实数据。非破坏微复现实证(activate→clear_if_resolved→`is_active False`)。修:`clear_if_resolved` 加 **30s 宽限期**(靠 `armed_at`,incident ~2s 内必建出,TTL 600s 仍兜底),宽限内不清。验证:宽限内 `True`(不清)/超期 `False`(自愈保留);活体注入 override 挺过 12s 并发。`sensing/override.py`。
- **③ CO2 屏幕一直红 = 真实贴阈值 + 坏帧**:查近 3h 真实 co2 avg **896**(正贴 900 WELL 阈值,在线两边来回穿)、范围含 **0 和 12509**(SCD30 冷启/I2C 毛刺)→ 屏幕边界闪红 + 偶发巨红。三连修:(a) **坏帧过滤**(`calibration.py` 物理边界 co2[250,5000]/temp[-10,60]/hum[0,100],越界丢弃记 NULL + warning;另清历史 9 行垃圾)·(b) **`latest_reading` 取每列最新非 NULL**(`history.py`,被丢的坏帧不把显示重置成 safe 默认,保持上一个真实值)·(c) **kiosk 判色滞回**(`kiosk.html`,红仍以 900 为准**不动 WELL 基准**,但要越/落 5% 余量才翻色,消边界闪;纯显示层)。**诚实边界**:滞回消的是「闪」,真实 co2 长期 880-920 仍稳定显红,变绿要通风压到 <855。
- **过程踩坑**:受控验证脚本放 `/tmp` 跑报 `No module named core`(sys.path[0]=脚本目录非 cwd)→ 设 `PYTHONPATH=$PWD`;真实 co2 高位时注入被真实单 dedup 挡 + 真实单 graph 线程已在飞、SQL 关单也停不掉 → 验证须先清真实单/停 scheduler;测试残留的 co2 override 让 `/api/sensors/current` 一度显示 sim 1300(**任何 override 活跃则 read_sensors 整体返回 sim**),已清 + sim_state 防御性重置为 650。
- 关联(本轮新增):`ops/scheduler.py`(verify gate)·`sensing/override.py`(grace)·`sensing/calibration.py`(坏帧过滤)·`sensing/history.py`(latest 非 NULL)·`frontend/app/kiosk.html`(判色滞回)·[[project-pi-bringup]]·[[project-hardware-deployment]]

### P-037 · Phase 5 · 展厅布展日:UCL_IoT 客户端隔离逼退真实传感器 + Tailscale 保住远程 + 语音三连修  🔧
- **背景**:2026-07-15 下午把展台从家里搬进展厅。三件事同时暴露:UCL 网络策略、我自己引入的 DNS 副作用、以及一个一直潜伏的语音时序 bug。展期临近,作者拍板「**只要稳定能用,不追求速度**」。
- **① 远程可达性先行(Tailscale)**:原 `ssh pi` 靠 Windows portproxy 打 Pi 的**家里网段 IPv6**,换网即死;而作者明确「到时候肯定需要你的帮助」。**搬走前**装 Tailscale(Pi + WSL 各一端,作者浏览器认证)。**决策正确性当天即验证**:Pi 到展厅后自动重连 tailnet,`ssh pi` 改指 `100.127.165.62` 后**全程远程排障、零现场介入**(含远程配 UCL_IoT WiFi)。走 DERP 中继(NAT 打洞失败,校园网正常现象),延迟 86-400ms,SSH 够用但偶尔卡。
- **② UCL_IoT 三问的答案(导师 Steve)+ 一个他没提的坑**:① 设备互通 **刻意隔离** → MKR **永远连不上 Pi 自己的 broker**,WiFi 直连方案死;建议改用公共 broker `mqtt.cetools.org` 中转。② **无固定 IP**(固件里 broker IP 写死 → 本就会挂)。③ 出站外网无限制(云 LLM/TTS 安全)。**实测发现他没提的**:`mqtt.cetools.org` **匿名只能订阅、不能发布**——`#` 订阅 14s 收 8151 条,但自己发的标记消息永不回流;**Mosquitto 对 ACL 拒绝的发布照回 PUBACK**,客户端完全看不出被拒(极阴的坑)。→ 走公共 broker 需要凭据(broker 上已有 `student/MUJI/hzh/` 主题,疑似作者旧课程有号)。
- **③ 数据源改判:全 sim + ambient(作者拍板)**:USB 串口方案(免网免凭据,但需重构固件——现固件**会卡死在 WiFi 循环、根本走不到串口打印那行**)与公共 broker 方案均需烧板;作者选择**放弃真实传感器、全用模拟数据**。**新 `sensing/ambient.py`**:日夜周期 + 多周期平滑漂移 + 高斯噪声,**live 与历史回填共用同一模型**(否则问答报「上周均温 22」而屏幕显 27 会当场穿帮);`in_band=True`(live)**钳在阈值内**——ambient 尖峰会开出执行器关不掉的单(执行器驱动的是 RoomState 的 ODE,不是 ambient 模型)→ 必 replan→FAILED;`in_band=False`(历史)保留会议尖峰/夜间黑暗,让「上周 CO2 峰值」有料。`RoomState.read_all()` 在 `sim_ambient` 且**无 override**时返回 ambient,**有 override(注入 demo)时返回真实物理**——demo 的 arm/执行器/verifier 完全不受影响。**开关默认关**:IEQ-Bench 读同一模拟器,必须保确定性(实测两次读数完全一致)。补 14 天历史填上 06-29~07-14 空档(2016 行,周末 CO2 明显低于工作日)。**诚信**:作者选择不在展签标注「模拟数据」,已明确告知其含义(展台摆着实物传感器,参观者会默认实时);真实链路确实做通过(在家跑三周),今日受阻是**UCL 刻意隔离**,非技术失败。
- **④ 语音「没声音」根因:TTS 时序,不是音响/STT/LLM**。作者先疑音响、再疑 STT,**全是无辜的**:测试音正常(闷是树莓派 3.5mm PWM 假 DAC 的固有素质)、`dashscope_transcribe` 识别精准、LLM 答案正确、TTS 冒烟单测 15 帧 222KB——**唯独串起来 0 帧**。根因:`synthesize_stream` **先 `tts.connect()` 再空等 LLM 首字**,DashScope 实时会话空闲即被服务端掐断(`websocket closed due to Connection timed out`),等首 token 回来连接已死,**且不报错**。**跟网速强相关**:家里首字 1.9s 侥幸不超时,热点 3.5s 必死。修:**先拉到第一个 token 再建连**,让会话开张第一件事就是发数据。验证 0 帧 → 19 帧/307KB。
- **⑤ 「有时候听不到」根因:我自己装的 Tailscale 的副作用**。`dashscope_transcribe_failed ... Temporary failure in name resolution`。**MagicDNS 接管了 Pi 的 DNS**,解析全绕 DERP 中继(300ms+),中继一抖解析就失败 → STT 请求根本发不出去。修:`tailscale set --accept-dns=false` 交回 UCL 本地解析器(144.82.250.1),实测 6/6 通过。**教训:Tailscale 只用它连通,别让它管 DNS**。
- **⑥ 稳定性加固(按作者「不追求速度」)**:STT 3 次重试 + TTS 建连 3 次重试(**只重试传输异常,不重试空转录**——空是「真的没说话」,重试白等)。
- **⑦ 量到但未修(作者指示先不动)**:**STT 8-16s**(`qwen3-asr-flash` 走「先传 WAV 到 OSS 再调模型」= 3 趟新加坡往返;冷启 76s)。可换 `paraformer-realtime-v2` 流式 WS 免上传(P-021 原方案),改动大留后。**浏览器识别不可用**(`browser_text=` 实测空 → Debian chromium 无 Google 语音 key),省 8s 的捷径堵死。**`asr_options={"language":"en"}` 未被 API 采纳**(转出过中文闲聊)。
- **⑧ UI(导师意见:「别人怎么知道按哪个按钮」)**:去掉打字输入/Send/Speak 勾选,只留**两个大按钮**——主按钮点一下开始、变红脉冲「⏹ Tap to send」再点结束;副按钮「✋ Interrupt」。**标签永远写明下一次点击会发生什么**,提示行随状态变。死代码(`askText`/`qInput`/`ttsToggle`)一并清除,acorn 验语法。
- **⑨ 其他**:Pi 中途掉线一次、作者拔电重启恢复;**journald 默认非持久,上一次启动日志随重启蒸发 → 掉线原因永久查不到**(已尝试开持久化日志但未确认成功,待补)。注入 demo 若被服务重启打断会留孤儿单 → `clear_if_resolved` 清不掉 override → 表盘锁死在注入值直到 TTL 600s(P-036 已记的现象,今日实撞)。
- **待跟进**:① 确认 `accept-dns=false` 扛住重启(未验证,拔电前没查完)② 注入瞬间 temp/lux 跟着跳(场景只定义 co2,余通道回落 RoomState 默认 22.5/420,参观者会看到「注入 CO2 温度也变」)→ 拟在 `arm()` 用当前 ambient 填充未定义通道(须 gate 在 sim_ambient,否则污染 bench)③ 持久化日志 ④ STT 提速 ⑤ mkr1010_node.ino 的 dtostrf 改动仍未提交(与 P-034「SAMD 无 dtostrf」结论矛盾,作者指示不管)
- **关联**:`sensing/ambient.py`(新)·`sensing/simulator/room.py::read_all`·`core/config.py::sim_ambient`·`ops/scripts/backfill_history.py`(复用 ambient)·`voice/provider.py`(TTS 时序 + STT/TTS 重试)·`frontend/api/main.py`(voice_stt 遥测)·`frontend/app/kiosk.html`(两按钮 UI)· Pi `.env`(SENSOR_SOURCE=sim/SIM_AMBIENT=true)· Pi `ieq-sampler.service`(新,venv python 非 uv run)·[[project-pi-bringup]]·[[project-exhibit-deployment]]·[[project-voice-cascade]]
